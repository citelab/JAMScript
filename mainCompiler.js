...
for (var i = 0; i < args.length; i++) {
  if (args[i].charAt(0) === "-") {
    ...
    if (extension === ".js") {
      ...
      if (path.extname(inputPath) !== ".jxe") supFiles.push(inputPath);
    }
  }
}
...
function flowCheck(input, verbose) {
  return new Promise(function (resolve, reject) {
    ...
    var hasFlow = child_process.execSync(
      "flow version >/dev/null 2>&1 || { echo 'not installed';}"
    );
    if (hasFlow.length === 0) {
      ...
      child_process.exec(command, (error, stdout) => {
        if (error !== null) {
          reject(
            "JavaScript Type Checking Error:\n" +
