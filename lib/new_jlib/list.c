...
void list_init(List *list) {
    ...
    list->tail = NULL;
    // list->size = 0;
    __atomic_store_n(&(list->size), 0UL, __ATOMIC_RELEASE);
}
...
size_t list_size(const List *list) {
    assert(list);
    // return list->size;
    return __atomic_load_n(&(list->size), __ATOMIC_ACQUIRE);
}

int list_empty(const List *list) {
    assert(list);

    return __atomic_load_n(&(list->size), __ATOMIC_ACQUIRE) == 0;
}
...
size_t list_index_of(const List *list, const ListNode *node) {
    ...
    if (list->tail == node) {
        // return list->size - 1;
        return __atomic_load_n(&(list->size), __ATOMIC_ACQUIRE) - 1;
    } else {
        ListNode *n;
        size_t i = 0;
...
ListNode* list_at(const List *list, size_t index) {
    ...
    assert(list && index < __atomic_load_n(&(list->size), __ATOMIC_ACQUIRE));
    // if (index < list->size / 2) {
    if (index < __atomic_load_n(&(list->size), __ATOMIC_ACQUIRE) / 2) {
        i = 0;
        list_for_each(n, list) {
            if (i == index) {
                return n;
        ...
        // i = list->size - 1;
        i = __atomic_load_n(&(list->size), __ATOMIC_ACQUIRE) - 1;
...
void list_splice_left(List *list, List *src_list, ListNode *position) {
    ...
    range_size = __atomic_load_n(&(src_list->size), __ATOMIC_ACQUIRE);
...
void list_remove_all(List *list) {
    ...
    list_cut(list, list->head, list->tail, __atomic_load_n(&(list->size), __ATOMIC_ACQUIRE));
}
...
void list_cut(List *list, ListNode *from, ListNode *to, size_t range_size) {
    ...
    __atomic_sub_fetch(&(list->size), range_size, __ATOMIC_ACQ_REL);
}
...
void list_paste(List *list, ListNode *left, ListNode *from, ListNode *to, ListNode *right, size_t range_size) {
    ...
    // list->size += range_size;
    __atomic_add_fetch(&(list->size), range_size, __ATOMIC_ACQ_REL);
}
...
void list_sort(List *list, int (*compare)(const ListNode *a, const ListNode *b)) {
    ...
    if (__atomic_load_n(&(list->size), __ATOMIC_ACQUIRE) < 2) {
        return;
    }
