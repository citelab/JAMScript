...
function remoteSyncExec(name, params, expr, vec, cback, callback) {
...
function remoteAsyncExec(name, params, expr, vec, cback, callback) {
    doRemoteAsyncExec(name, params, expr, vec, false, cback, callback);
}


function remoteAsyncExecCB(name, params, expr, vec, cback, callback) {
    doRemoteAsyncExec(name, params, expr, vec, true, cback, callback);
}


function doRemoteAsyncExec(name, params, expr, vec, cbmode, cback, callback) {
    ...
    if ((mlevel === undefined) ||
        ...
        var tmsg = {"cmd": "REXEC-ASY", "opt": machtype.toUpperCase(), "cond": processExpr(expr), "condvec": vec,  "actname": name, "actid": "-", "actarg": "-", "args": params};
...
function machSyncExec(name, params, oexpr, vec, cback, callback) {
    ...
    else
    {
        ...
        var tmsg = {"cmd": "MEXEC-SYN", "opt": machtype.toUpperCase(), "cond": oexpr, "condvec": vec, "actname": name, "actid": "-", "actarg": "-", "args": params};
...
//
function machAsyncExec(name, params, oexpr, vec, cback) {
