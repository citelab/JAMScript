...
class JCoreClient {
    ...
    remoteSyncExec(name, params, expr, vec, bclock, scount, mlevel, cbid) {
        ...
        if ((mlevel === undefined) ||
            ...
            this.jcore.results = [];
            this.jcore.cNodeCountACK = 0;
            this.jcore.cNodeCountNAK = 0;
            this.jcore.deviceIds.clear();
            this.jcore.startEnabled = false;
    ...
    remoteSyncCtxExec(name, params, expr, vec, bclock, scount, mlevel, cbid, reexec) {
        if ((mlevel === undefined) ||
            (mlevel === 4 && this.machtype === globals.NodeType.CLOUD) ||
            (mlevel === 2 && this.machtype === globals.NodeType.FOG) ||
            (mlevel === 1 && this.machtype === globals.NodeType.DEVICE)) {
            ...
            const ctxJob = () => {
                ...
                const contextId = contextManager.getContextId();
                const context = contextManager.getContext(contextId);
                context.running = true;
                context.start.then(() => {
                    if (jsys.id === context.executor && !context.done.fulfilled) {
                        logger.info(__filename, 'main', `Executing J2C sync activity for context ${contextId}`);

                        if (reexec || context.reexec) {
                            this.jcore.sendSyncStart(-1);
                        }

                        const tmsg = JAMP.createRemoteSyncReq(name, params, expr, vec, this.machtype, bclock, scount);
                        tmsg.cbid = cbid;
                        ...
                        const checkDone = () => {
                            if (fogDone && devDone) {
                                ...
                                delete this.jcore.context;
                                if (!ctxErr) {
                                    contextManager.writeDone(contextId, ctxRes, undefined);
                                } else {
                                    contextManager.writeDone(contextId, undefined, ctxErr);
                                }
                            }
                        };

                        const callback = rstatus => {
                            switch (rstatus.code) {
                                case 'ACK':
                                    this.jcore.runTable.processAck(tmsg.actid, 'device');
                                    break;
                                case 'RES':
                                    setDevDone(rstatus.res, undefined);
                                    checkDone();
                                    break;
                                case 'ERR':
                                    setDevDone(undefined, 'Time-out-error');
                                    checkDone();
                                    break;
                                case 'CANCEL':
                                    setFogDone();
                                    setDevDone(undefined, 'Cancellation');
                                    checkDone();
                                    break;
                            }
                        };

                        this.jcore.runTable.insert(
                            tmsg.actid, // runid
                            tmsg,
                            cbid,
                            [{device: this.jcore.mserv, topic: `/${cmdopts.app}/mach/func/request`}], // targets
                            callback
                        );

                        this.jcore.context = {contextId, cbid, callback, setFogDone, setDevDone, checkDone};
                    }
                });
                context.done.then(
                    res => {
                        contextManager.clearContext(contextId);
                        contextManager.setContextId(contextId + 1);
                        this.jcore.enqueueJob({cmd: 'REXEC-RES', data: {device: res}, cbid, ctxid: contextId});
                        if (this.ctxQueue.length > 0) {
                            const ctxJob = this.ctxQueue.shift();
                            ctxJob();
                        } else {
                            this.ctxBusy = false;
                        }
                    ...
                    err => {
                        if (err === 'context-restart') {
                            this.remoteSyncCtxExec(name, params, expr, vec, bclock, scount, mlevel, cbid, true);
                        } else {
                            contextManager.clearContext(contextId);
                            contextManager.setContextId(contextId + 1);
                            this.jcore.enqueueJob({cmd: 'REXEC-ERR', data: {device: err}, cbid, ctxid: contextId});
                            if (this.ctxQueue.length > 0) {
                                const ctxJob = this.ctxQueue.shift();
                                ctxJob();
                            } else {
            ...
            if (!this.ctxBusy || reexec) {
                this.ctxBusy = true;
                ctxJob();
            } else {
                this.ctxQueue.push(ctxJob);
            }
        }
    }
