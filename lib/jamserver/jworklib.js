...
module.exports = new function() {
    ...
    this.remoteSyncCtxExec = remoteSyncCtxExec;
...
function remoteSyncCtxExec(name, params, expr, vec, bcasters) {
    const callback = params.pop();
    if (callback != null && typeof callback !== 'function') {
        return Promise.reject("Invalid callback");
    }

    const bclock = getMaxBcastClock(bcasters);
    if (bclock === null) {
        return Promise.reject("Broadcast clock error");
    }

    const mlevel = curLevel();
    scount++;
    const cbid = name + scount;
    ...
    ctxCallbacks.set(cbid, {callback, onFulfilled: res, onRejected: rej});
    postMessage({cmd: 'REXEC-SYN', name, params, expr, vec, bclock, count: scount, level: mlevel, cbid, ctxid: -1});
...
function run(callback) {
    ...
    onmessage = function(ev) { 
        ...
        switch (v.cmd) {
            ...
            case 'REXEC-ERR':
                if (v.ctxid != null) {
                    processCtxWait(v.cbid, v.data, true);
                } else {
            ...
            case 'REXEC-RES':
                if (v.ctxid != null) {
                    processCtxWait(v.cbid, v.data, false);
                } else {
                    processWait(v.cbid, v.data, false);
                }
                postMessage({cmd: 'DONE'});
            break;
            ...
            case 'CTX-CALLBACK':
                processCtxCallback(v.cbid, v.args);
                postMessage({cmd: 'CTX-CALLBACK-DONE'});
            break;
...
function asyncExecute(v, doneMsg, errMsg) {
    ...
    if (checkCondition(v.cond) !== true)
        postMessage({cmd: doneMsg, data: "", actid: v.actid});
...
function processCtxCallback(cbid, args) {
    const entry = ctxCallbacks.get(cbid);
    if (entry === undefined) {
        return;
    }
    const func = entry.callback;
    if (func) {
        try {
...
function processCtxWait(cbid, data, error) {
    const entry = ctxCallbacks.get(cbid);
    if (entry === undefined) {
        return;
    }
    if (!error) {
        entry.onFulfilled(data.device);
    } else {
        entry.onRejected(data.device);
    }
    ctxCallbacks.delete(cbid);
}
