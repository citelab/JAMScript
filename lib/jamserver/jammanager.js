...
const ncache = require('./jnode').ncache();

<<<<<<< HEAD:lib/jserver/jammanager.js
// get the hostname
var sync_exec = require('sync-exec');
var res = sync_exec("hostname -I").stdout;
var host = res.split(" ")[0];
...
=======
>>>>>>> JAMScript-beta/master:lib/jamserver/jammanager.js

// Allows other machine to access redis clients on this one.
sync_exec("redis-cli config set protected-mode no");
...
module.exports = (function(host, port) {
    ...
    listener.on('error', function() {
        console.log("----------------__CONNECTION ERROR---------");
    });
...
//    var parentRedisLogger = null;   //This is for logging data to the parent
    ...
    ncache.onFogDataUp(function(info) {
    ...
    ncache.onFogDataDown(function(info) {

        if (pack.isDevice && (parentConObj !== null))
            doDisconnectFromParent();
    });

    ncache.onCloudDataUp(function(info) {
        console.log("Cloud Data Up ------------------------", info);
        if( pack.isFog && (parentConObj == null || parentConObj.host != info.host || parentConObj.port != info.port) )
            doConnectToParent(info);
    });

    ncache.onCloudDataDown(function(info) {

        if (pack.isFog && (parentConObj !== null))
            doDisconnectFromParent();
    });
    ...
    //    parentRedisLogger = pack.connectToParent(info.host, info.port);
        ...
        parentRedis.on('closesock', function() {
            parentConObj = null;
        });

    //    parentRedisLogger.on('closesock', function() {
//            parentConObj = null;
//        });
    }
    ...
    function init() {
        ...
        // E: Keyspace events
        // z: Sorted sets
        listener.rawCall(['config', 'set', 'notify-keyspace-events', 'KEA']);
