...
class JAMCore {
    ...
    setWorker(worker) {
        ...
        this.ncache.onFogDataUp(function(info) {
            ...
            that.learner.postMessage({cmd: 'NCACHE-MOD', opt: 'FOG-DATA-UP', data: info});
        });

        this.ncache.onFogDataDown(function() {
            that.worker.postMessage({cmd: 'NCACHE-MOD', opt: 'FOG-DATA-DOWN'});
            that.learner.postMessage({cmd: 'NCACHE-MOD', opt: 'FOG-DATA-DOWN'});            
        });
        ...
        //    console.log("---------- CLOUD DATA UP-----------");
            ...
            that.learner.postMessage({cmd: 'NCACHE-MOD', opt: 'CLOUD-DATA-UP', data: info});            
        });

        this.ncache.onCloudDataDown(function() {
         //   console.log("---------- CLOUD DATA UP-----------");
            that.worker.postMessage({cmd: 'NCACHE-MOD', opt: 'CLOUD-DATA-DOWN'});
            that.learner.postMessage({cmd: 'NCACHE-MOD', opt: 'CLOUD-DATA-DOWN'});            
        });
    }
