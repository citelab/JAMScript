...
module.exports = new function parseArgs() {
    ...
    if (options.zone) {
        const parts = options.zone.split(/[,:]/);
        switch (parts.length) {
            case 1: // --zone=z1
                options.zone = parts[0];
                options.ctxlog_host = '127.0.0.1';
                options.ctxlog_port = 6379;
                break;
            case 2: // --zone=z1,192.17.9.24
                options.zone = parts[0];
                options.ctxlog_host = parts[1];
                options.ctxlog_port = 6379;
                break;
            case 3: // --zone=z1,192.17.9.24:6512
                options.zone = parts[0];
                options.ctxlog_host = parts[1];
                options.ctxlog_port = parseInt(parts[2]);
                break;
        }
    }
