...
<<<<<<< HEAD:lib/jserver/jamdatastream.js
=======
const cmdopts = require('./cmdparser');

>>>>>>> JAMScript-beta/master:lib/jamserver/jamdatastream.js
class JAMDatastream {
...
<<<<<<< HEAD:lib/jserver/jamdatastream.js
        if (e) {
            ...
            for (var i = 0; i < response.length; i++) {
                ...
                try{
                    log = cbor.decodeFirstSync(parts[0]);
                    if(typeof log != 'object') log = parts[0];
                }catch(e){
                    //console.log("not cbor encoded");
                    log = parts[0];
                }
        ...
        if (e) {
            ...
            for (var i = 0; i < response.length; i++) {
                ...
                entry = {
                    ...
                    time_stamp: parts.length > 3 ? parts[3] : parts[2]
                };
                datastream.data_values.push(entry);
                datastream.index_of_last_value++;
                datastream.set_size++;
...
<<<<<<< HEAD:lib/jserver/jamdatastream.js
                        listener.notify.call(listener, datastream.key, entry);
                    else if( typeof listener === 'function' )
                        listener.call({}, datastream.key, entry);
                }
            }
...
=======
                ...
                if( forward ) {
                    ...
                    var slice = parts.slice(0);
                    slice[0] = typeof entry.log === "object" ? JSON.stringify(entry.log) : entry.log;
                    //console.log("Before forwarding:", slice[0]);
                    //console.log("---------------------------------------------- forwarding..--------");
                    datastream.jammanager.simpleLog(datastream.key, slice.join(datastream.delimiter), null, datastream.jammanager.getParentRedisLogger(), (parts.length > 3 ? parts[4] : parts[3]) - 0);
                }
            }
