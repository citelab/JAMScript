...
class JAMDatastream {
    ...
    constructor(dev_id, key, fresh, jammanager, data_prototype, refresh_rate, slots, redis) {
        ...
        this.isBusy = false;
        this.hasData = false;
        this.isLocalStream = false; //If this stream is created by the datasource's getMyDataStream method
        this.isCBOR = false;

        this.redis = redis ? redis : Redis.createClient({
            "host": cmdopts.redhost,
            "port": cmdopts.redport
        });
        if (jammanager) {
        ...
        this.fresh = fresh;
        this.index_of_last_value = 0; //So we can zrange without having to retrieve everything
        ...
        // }
        this.request_value_refresh(true);   //force data refresh the first time the datastream is created
    ...
    async getLastValueSync(timeout){
        ...
        let promise = new Promise(resolve => {
            var f = function(){
                if( self.getLastValue() === null )
                    setTimeout(f, 100);
                else
                    resolve(self.getLastValue());
            };
            f();
        });
        if( timeout ){
            return await Promise.race([new Promise((resolve, reject) => {setTimeout(reject, timeout)}), promise]).catch(error => null);
        }
        else {
    ...
    request_value_refresh(force) {
        ...
        this.isBusy = true;
        this.hasData = false;
        this.zrange(this.key, this.index_of_last_value, -1, 'withscores',
            function(e, response) {
                var pairs = response.reduce(function(result, value, index, array) {
                    if (index % 2 === 0)
                      result.push(array.slice(index, index + 2));
                    return result;
                  }, []);
                process(e, pairs, datastream, this.isCBOR);
            }
        );
        if (this.refresh_rate > 0) {
    ...
    process_zrange_response(e, response, datastream, isCBOR) {
        var parts;
        var entry;
        ...
        if (e) {
            ...
            for (var i = 0; i < response.length; i++) {
                ...
                if( log.substring(0, 1) === "{" ){
                    ...
                    var blob = Buffer.from(log, 'base64');
                    try {
                        log = cbor.decodeFirstSync(blob);
                ...
                if( forward ) {
                    ...
                    var slice = response[i].slice(0);
                    slice[0] = typeof entry.log === "object" ? JSON.stringify(entry.log) : entry.log;
                    //console.log("Before forwarding:", slice[0]);
                    //console.log("---------------------------------------------- forwarding..--------");
                    datastream.jammanager.simpleLog(datastream.key, slice[0], null, datastream.jammanager.getParentRedisLogger(), timeStamp);
                }
            }
    ...
    zrange(key, start, range, option, callback) {
        this.redis.zrange([key, start, range, option], callback);
    }
    ...
    log(entry, callback, redis) {
        ...
        redis.zadd([this.key, curTime, entry], function(e, d) {
            if (e) {
                if (callback) {
                    callback({
                        status: false,
