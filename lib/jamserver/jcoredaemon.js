...
class JCoreDaemon {
    ...
    runSyncCallbackRTE(cmsg, callback) {
    	...
    	else
    	{
    	    ...
    	    var nmsg = {"cmd": "REXEC-RES", "opt": this.machtype.toUpperCase(), "cond": "-", "condvec": 0, "actname": cmsg["actname"],
            ...
            console.log("-------->>> Sending results back... ", res);
    	}
    }
    ...
    runAsyncCallback(cmsg, callback) {
    	...
    	if (fentry === undefined) {
            ...
            rmsg.args = ["ILLEGAL-PARAMS"];
    		callback(rmsg);
    	} else if (this.checkCondition(cmsg["cond"]) !== true) {
            rmsg.cmd = "REXEC-NAK";
            rmsg.args = ["CONDITION-FALSE"];
    		callback(rmsg);
        } else {
