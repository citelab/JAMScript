...
class JCoreDaemon {
    ...
    machRunner(sock, msg) {
        ...
        } else if (msg['cmd'] === 'ZONE-CONF') {
            if (jsys.seqNum === undefined || jsys.seqNum < msg.seqNum) {
                logger.debug(__filename, 'main', 'Received message:', msg);
                jsys.seqNum = msg.seqNum;
                jsys.primaryFog = msg.primaryFog;
                jsys.secondaryFog = msg.secondaryFog;
            }
        }
    }
    ...
    machService(msg) {
        /*
        // simulate device sending NAK
        if (msg['cmd'] === 'REXEC-ACK') {
            msg['cmd'] = 'REXEC-NAK';
        }
        */
...
//                    console.log("Resmap size ", this.jcore.resMap.size, " NodeCount ", this.jcore.cNodeCountACK);

                    // Received results from all C nodes.
                    if (this.jcore.resMap.size >= this.jcore.cNodeCountACK) {
