...
class JCoreAdmin {
    ...
    doRegister() {
        ...
        this.ncache.onCloudUp(function(id, info) {
            ...
            // it is undefined..
            if (that.jcore.cserv === undefined && id !== undefined && info !== undefined) {
                that.cid = id;
                var curl = getURL(info.ip, info.port);
                that.jcore.cserv = mqtt.connect(curl);
        ...
        this.ncache.onCloudDown(function(id, info, callback) {
            ...
            if (that.jcore.cserv !== undefined && id !== undefined && info !== undefined) {
                ...
                that.jcore.cserv = undefined;
                if (that.machtype === globals.NodeType.FOG)
                    deviceParams.setItem('parentId', '-');
                console.log("Cloud down....");
                ebus.emitCloudDown(that.cid, info);
                jsys.unsetCloud(that.cid);
            }
        });
