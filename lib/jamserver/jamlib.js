...
jamsys.init(reggie, machType, cmdopts.tags, cmdopts.iflow, 
	cmdopts.oflow, deviceParams.getItem('deviceId'),
								cmdopts.link, cmdopts.zone, cmdopts.long, cmdopts.lat);
jamsys.setMQTT(getMachineAddr(), cmdopts.port);
var jnode = require('./jnode');

if (jamsys.type === globals.NodeType.FOG && cmdopts.zone) {
    const ContextManager = require('./ctxmgr');
    const contextManager = new ContextManager(jamsys.id, cmdopts.app, cmdopts.zone, cmdopts.ctxlog_host, cmdopts.ctxlog_port);
    contextManager.initialize();
    jamsys.setContextManager(contextManager);
}
