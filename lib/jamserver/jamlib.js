...
deviceParams.setItem('port', cmdopts.port);
...
var reggie = new Registrar(cmdopts.app, machType, deviceParams.getItem('deviceId'), cmdopts.port, {mqtt: false, mdns: true, localStorage: false});

var jamsys = require('./jamsys');
jamsys.init(reggie, machType, cmdopts.tags, deviceParams.getItem('deviceId'));
...
function run(callback) {
	...
	checkMQTTServer(function(present) {
        ...
        if (machType === globals.NodeType.FOG)
            reggie.addAttributes({testdata: "I am fog... blah blah "});
        else
        if (machType === globals.NodeType.CLOUD)
            reggie.addAttributes({testdata: "I am cloud ... blah blah "});
