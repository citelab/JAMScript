...
function zoneFogSelector() {
    if (fogs.size === 0) {
        return undefined;
    }

    let fogId = sys.primaryFog;
    if (fogs.has(fogId)) {
        return [fogId, fogs.get(fogId)];
    }

    fogId = sys.secondaryFog;
    if (fogs.has(fogId)) {
        return [fogId, fogs.get(fogId)];
    }
...
module.exports = function(idelay, dstep, fdelay, linkp) {
    ...
    this.fogDown = function(id) {
        ...
        if (id === currentFog) {
            ...
            if (fval !== undefined) {
                ...
                if (nfog !== undefined) {
                    logger.info(__filename, 'main', 'Detected fog down. Connecting to fog:', nfog[1].key);
    ...
    var selectTimer = setTimeout(function runSelection() {
        ...
        if (currentFog === undefined) {
            var nfog = fogSelector();
            if (nfog !== undefined) {
                logger.info(__filename, 'main', 'Initially connecting to fog:', nfog[1].key);
        ...
        } else if (selPolicy === 'near' || selPolicy === 'zone') {
            var nfog = fogSelector();
            if (nfog !== undefined && currentFog !== nfog[1].key) {
                logger.info(__filename, 'main', 'Connecting to fog:', nfog[1].key);

                // disconnect from the old one, connect to the new one
