...
class JAMDatasource {
    ...
    getPrivateStream(deviceId) {
        const privateStreamId = 'private_' + deviceId;
        const predicate = streamId => streamId.startsWith(deviceId);
        return this.getDerivedStream(privateStreamId, predicate);
    }

    getSharedStream(context) {
        const sharedStreamId = 'shared';
        let predicate;
        if (context) {
            const streamIds = new Set();
            for (const deviceId of context.deviceIds) {
                streamIds.add(deviceId);
                streamIds.add(deviceId.split(/-(?=[^-]+$)/)[0] + '_device');
            }
            predicate = streamId => streamIds.has(streamId);
        } else {
            predicate = streamId => false;
        }
        return this.getDerivedStream(sharedStreamId, predicate);
    }

    getDerivedStream(derivedStreamId, predicate) {
        let derivedStream = undefined;

        const deviceStreams = [];
        const numStreams = this.size();
        for (let i = 0; i < numStreams; i++) {
            const stream = this[i];
            const streamId = stream.getDeviceId();
            if (predicate(streamId)) {
                deviceStreams.push(stream);
            } else if (derivedStreamId === streamId) {
                derivedStream = stream;
            }
        }
        deviceStreams.sort((stream1, stream2) => {
            const id1 = stream1.getDeviceId();
            const id2 = stream2.getDeviceId();
            return id1 < id2 ? -1 : (id1 > id2 ? 1 : 0);
        });
        Object.freeze(deviceStreams);

        if (!derivedStream) {
            derivedStream = this.addDatastream(derivedStreamId);
        }

        if (!derivedStream.logFunc) {
            derivedStream.logFunc = derivedStream.log;
        }

        derivedStream.log = (func, ...args) => {
            const entry = func.call(derivedStream, deviceStreams, ...args);
            derivedStream.logFunc.call(derivedStream, entry);
        };
