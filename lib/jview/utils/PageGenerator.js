...
module.exports = function (pages, path) {
    ...
    pages.map(page=> {
        ...
        if (page.name) {
            fs.mkdir(__PageDirectory(page.name, path), function(err) {
                if(err) throw err;
                ...
                page.panels.map((panel, idx)=> {
                    ...
                    if (panel.type == 'formset') {
                        ...
                        panelStoreConnectors.push("const " + storeName + "_controller = this.props."+storeName+".controller;\nconst " + storeName + "Config = {\
                            reset: this.props." + storeName + ".reset.bind(this.props." + storeName + "),\
                            changeValue: this.props." + storeName + ".changeValue.bind(this.props." + storeName + "),\
                            emitValue: (id, value = 0) => {this.socket.emit('emitValue', {id : id, value: value})},\
                ...
                fs.writeFile(__RootFileName(page.name, path), '// Root File for Page ' + page.name, function (err, data) {
                    if (err) throw err;
                    ...
                    if(sockets.length != 0){
                        ws.writeLine("componentDidMount(){\
                            if(!this.socket) {\
                                ...
                                this.socket.on('changeValue', body => {\
                                    this.props." + storeName + ".changeValue(body.value, body.name)\
                                });\
    ...
    fs.writeFile(path + '/Main.js', '// Root File for The Tree Flow app', function (err) {
        if (err) throw err;
