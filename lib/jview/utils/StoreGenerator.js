            ...
            if(panel.controlList){
                panel.controlList.forEach(e=>{
                    if (e.type==="slider") {
                        ...
                        ws.writeLine('  "trigger": "changeValue",')
                        ws.writeLine('  "valueName": "' + e.valueName + '"')
                        ws.writeLine("}")
                    }
                })
                //ws.writeLine("const controlList = "+JSON.stringify(panel.controlList, null, 4));
            } else {
...
emitValue(id, value){socket.emit('emitValue', {id : id, value: value})}"
