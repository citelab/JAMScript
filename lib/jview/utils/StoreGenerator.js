...
module.exports = function(storeName, panel, pageDirectory){
    ...
    fsPath.writeFile(__StoreFileDir(pageDirectory,storeName), '//Store File for '+storeName, function(err,data){
        ...
        ws.writeLine(__StoreFileDependencies);
        ws.writeLine(__ClassName(storeName));

        //write
        ws.writeLine(__BasicFunctions);
        if (panel.type == 'formset'){
            ...
            ws.writeLine("addDataPoints (x,y,gateIndex){this.array[gateIndex].push([x,y])};");
...
const __StoreFileDependencies = "import { autorun, observable, computed} from 'mobx';"

const __BasicFunctions = "reset(){this.map=this.map.map(e=>{return 0})}\
