    ...
    render(){
        ...
        const ControllerItem = function(item,idx) {
            ...
            switch (item.type){
                ...
                case 'terminal':
                    return (
                        <TerminalController { ...item } trigger={trigger} buttonTrigger={props.addCommand} displayLabel={displayLabel} key={idx}/>
                    )
                    break
                case 'multiStateButton':
                    return (
                        <MultiStateButton { ...item } trigger={trigger} displayLabel={displayLabel} key={idx}/>
                    )
            }
        }
