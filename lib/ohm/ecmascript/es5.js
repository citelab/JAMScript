...
function binaryExprPrint(node) {
  if(node.children.length == 3) {
    return node.child(0).prettyish + ' ' + node.child(1).interval.contents + ' ' + node.child(2).prettyish;
  } else {
...
function unaryExprPrint(node) {
  if(node.children.length == 2) {
    return node.child(0).interval.contents + ' ' + node.child(1).prettyish;
  } else {
    return node.prettyish;
  }
}
...
var prettyish = {
  ...
  //   return id.interval.contents + init.prettyish;
  // },
  ...
  ContinueStatement: function(_1, _2, id, _4) {
    ...
    if(id.numChildren == 1) {
      return "continue " + id.child(0).interval.contents + ";";
    } else {
      return "continue;";
    }
  },
  BreakStatement: function(_1, _2, id, _4) {
    ...
    if(id.numChildren == 1) {
      return "break " + id.child(0).interval.contents + ";";
    } else {
      return "break;";
    }
  },
