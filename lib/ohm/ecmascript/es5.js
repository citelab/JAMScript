...
function insertYieldPoint(expr, stmt, evalExpr) {
  if (evalExpr) {
    expr = expr.es5Translator;
  }
  var hasVar = false;
  var result = {
    "expr": expr,
  ...
  // Check if the expr contains variables
  try {
      eval(expr);
  } catch(e) {
      hasVar = e.name === 'ReferenceError';
  }

  // If the expr does not contain variables and it constantly evaluates to true
  // Then we insert a yield point to the loop body 
  if (!hasVar && expr) {
    if(stmt.child(0).ctorName === "Block") {
      result.stmt = "{\n" + stmt.child(0).child(1).es5Translator + "\njsleep();\n}";
    } else {
...
var es5Translator = {
  ...
  VariableStatement: function(_1, decList, _2) {
    return "var " + decList.es5Translator + ";";
  },
  ...
  IterationStatement_doWhile: function(_1, stmt, _2, _3, exp, _4, _5) {
    var result = insertYieldPoint(exp, stmt, true);
    return "do " + result.stmt + " while (" + result.expr + ");";
  },
  IterationStatement_whileDo: function(_1, _2, exp, _3, stmt) {
    var result = insertYieldPoint(exp, stmt, true);
    return "while (" + result.expr + ") " + result.stmt;
  },
  IterationStatement_for3: function(_1, _2, exp1, _3, exp2, _4, exp3, _5, stmt) {
    var exp1 = exp1.es5Translator;
    var exp2 = exp2.es5Translator;
    var exp3 = exp3.es5Translator;
    var exp = false;
    if (exp1.length == 0 && exp2.length == 0 && exp3.length == 0) {
      exp = true;
    } 
    var result = insertYieldPoint(exp, stmt, false);
    return "for (" + exp1 + "; " + exp2 + "; " + exp3 + ") " +  result.stmt;
  },
  IterationStatement_for3var: function(_1, _2, _3, vars, _4, exp1, _5, exp2, _6, stmt) {
    return "for (var " + vars.es5Translator + "; " + exp1.es5Translator + "; " + exp2.es5Translator + ") " +  assureBlock(stmt);
  },
  IterationStatement_forIn: function(_1, _2, lhs_exp, _4, exp, _6, stmt) {
    return "for (" + lhs_exp.es5Translator + " in " + exp.es5Translator + ") " +  assureBlock(stmt);
  },
  IterationStatement_forInVar: function(_1, _2, _3, vars, _4, exp, _6, stmt) {
    return "for (var " + vars.es5Translator + " in " + exp.es5Translator + ") " +  assureBlock(stmt);
  },
