...
function binaryExprPrint(node) {
	if (node.children.length === 3) {
		...
		return (
			node.child(0).es5Translator +
			" " +
			node.child(1).sourceString +
			" " +
			node.child(2).es5Translator
		);
	} else {
...
var es5Translator = {
	...
	PostfixExpression: function (node) {
		if (node.numChildren === 3) {
			return node.child(0).es5Translator + node.child(2).sourceString;
		} else {
			return node.es5Translator;
		}
	},
	...
	IterationStatement_for3: function (
		...
		return (
			...
			exp3.es5Translator +
			") " +
			insertYieldPoint(stmt)
		);
	},
	IterationStatement_for3var: function (
		_1,
		_2,
		_3,
		vars,
	...
	CaseBlock_withDefault: function (
		_1,
		beforeCases,
		defaultClause,
		afterCases,
	...
	_iter: function (...children) {
		return children.map((c) => c.es5Translator);
	},
	_terminal: function () {
	...
	NonemptyListOf: function (first, sep, rest) {
		...
		for (var i = 0; i < rest.numChildren; i++) {
			code += sep.child(i).sourceString + " " + rest.child(i).es5Translator;
		}
		return code;
	},
