var taskMatrix = require("./taskMatrix.json");
...
module.exports = {
	...
	addTask: function (language, name, type) {
		if (!this.graph[language].has(name)) {
			this.graph[language].set(name, {
				type: type,
				calls: new Map(),
	...
	pruneJSCallGraph: function () {
		this.graph.js.forEach(function (target, sourceFunction, map) {
			...
			for (let call of calls.keys()) {
				if (
					!(
						symbolTable.functions.js.has(call) ||
						symbolTable.task.js.has(call) ||
						symbolTable.task.c.has(call)
					)
				) {
					calls.delete(call);
				}
	...
	checkCalls: function () {
		...
		//       if(!taskMatrix['js'][data.type][targetSymbol.language][targetSymbol.taskType]) {
		//         throw(`Cannot call ${targetSymbol.language} ${targetSymbol.taskType} task ${call} from ${language} ${data.type} task ${sourceFunction}`);
		//       }
		//     }
		//     if(targetSymbol.language === "js" && targetSymbol.taskType === "sync") {
		//       throw(`Cannot call javascript synchrnous task ${call} from function ${sourceFunction}, javscript sync calls must be at top level`);
		//     }
		//   }
		// });
				...
				if (data.type !== "function") {
					for (let call of data.calls.keys()) {
						...
						if (targetSymbol !== "function") {
							if (
								!taskMatrix["js"][data.type][targetSymbol.language][
									targetSymbol.taskType
								]
							) {
								throw `Cannot call ${targetSymbol.language} ${targetSymbol.taskType} task ${call} from ${language} ${data.type} task ${sourceFunction}`;
							}
	...
	createWebpage: function () {
		...
		output +=
			'<script src="https://cdnjs.cloudflare.com/ajax/libs/cytoscape/2.7.24/cytoscape.js"></script>\n';
		output +=
		...
		if (this.graph.c.size > 0) {
			this.graph.c.forEach(function (target, source) {
				...
				calls.forEach(function (arguments, call) {
					...
					output += `{data: {id: "${
            source + call
          }", source: "${source}", target: "${call}", calls: decodeURIComponent("${encodeURIComponent(
            tooltipTable
          )}")}},\n`;
				});
			});
		...
		if (this.graph.c.size > 0) {
			...
			this.graph.c.forEach(function (data, source, map) {
				...
				data.calls.forEach(function (arguments, call) {
					...
					arguments.forEach(function (args) {
						callList += source + " -> " + call + ' [ label="' + args.replace(/"/g, "'") + '" ];\n';
					});
				});
			});
