    ...
    Jamarray_decl: function(node) {
        ...
        for (var decl of node.decls) {
            ...
            if (init.initlen > 0) {
                ...
                e = astify(`NVOID_STATIC_INIT(${[init.name.name, atype, 0, '${types.get(node.jamtype).jamlib}', init.size, init.initlen]});`, "Expr_stmt");
                e.expr.args.push(init.init);
            } else
                e = astify(`NVOID_STATIC_INIT_EMPTY(${[init.name.name, atype, 0, '${types.get(node.jamtype).jamlib}',init.size]});`, "Expr_stmt");
...
function translate(ast, hasJ) {
    let decls = hasJ ? jdata.createCVariables(symbolTable.jdata) : [];
