...
var jamCTranslator = {
  ...
  Jtask_activity: function(returnType, _, condSpecifier, attrSpecifier, declarator, statement){
    ...
    const functionWrapper = activities.generateCActivities(retType, functionName, functionParams);

    symbolTable.addActivity(functionName, {
      language: 'c'
    });

    return `${retType} ${functionName}(${params})${functionStatement}\n${functionWrapper}\n`;
  },
  // params
...
function generate_taskmain() {
  var cout = `\nvoid taskmain(int argc, char **argv) {\n
    ...
    cnode_start(cnode);
    ${symbolTable.activities.c.keys.map((key) => {
      `registerFunction(cnode, "${symbolTable.activities.c[key]}", false);`
    })}
  }\n`;
  return cout;
}
