...
const translateNamespaceAccess = {
    ...
    Struct_access_expr: function(node) {
        if (node.struct.type !== "identifier")
            node.struct = this.walk(node.struct);
        if (node.struct.type === "identifier") { // Potentially namespace access
...
function translate(ast, hasJ) {
    ...
    decls = types.generateArrayDefs(decls);
    for (const jTask of symbolTable.tasks.j.values())
...
module.exports = {
	...
	compile: function (ast, yieldPoint, jCond, hasJdata) {
		jCondMap = jCond;
		//exportLibs = libs;
