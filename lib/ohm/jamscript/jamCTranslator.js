	...
	if (left.child(0).ctorName === "Left_expr_Member") {
		...
		if (typeof(pobj.type_spec) === 'string') {
			...
			switch (pobj.type_spec) {
				case 'int':
					return (flowfn === 'write' ? `uwrite_int(${psrc}, ${param})` : `dread_int(${psrc}, ${param})`);
				break;
				case 'char*':
					return (flowfn === 'write' ? `uwrite_str(${psrc}, ${param})` : `dread_str(${psrc}, ${param})`);
				break;
				case 'float':
				case 'double':
					return (flowfn === 'write' ? `uwrite_double(${psrc}, ${param})` : `dread_double(${psrc}, ${param})`);
				break;
			}

		} else if (typeof(pobj.type_spec) === 'object') {
			console.log("Secondary type ", pobj.type_spec.entries);
			let structCall = jdata.createFormatAndArray(param, pobj.type_spec.entries);
			if (flowfn === 'write') 
				return `uwrite_struct(${psrc}, "${structCall.formatString}", ${structCall.valueArray.join(", ")})`;
			else
				return `dread_struct(${psrc}, "${structCall.formatString}", ${structCall.valueArray.join(", ")})`;
		}
	}
	return left.cTranslator + call.cTranslator;
};
