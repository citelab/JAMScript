...
const translateNamespaceAccess = {
    ...
    Jamarray_decl: function(node) {
        this.declBaseType.push(node.jamtype);
        this.walk(node.decls);
        this.declBaseType.pop();
        return node;
    },
    Jamlist_decl: function(node) {
        this.declBaseType.push(node.jamtype);
        this.walk(node.decls);
        this.declBaseType.pop();
        return node;
    ...
    Jamarray_init: function(node) {
        this.inDecl++;
        if (node.size)
            this.declSubType.push({static: true, size: node.size});
        else
            this.declSubType.push({static: false});

        this.walk(node.name);

        this.declSubType.pop();
        this.inDecl--;

        this.walk(node.init);
        return node;
    },
    Param_decl_JamArray: function(node) {
        this.inDecl++;
        this.declBaseType.push(node.jamtype);
        this.declSubType.push({static: true, size: node.size});
    ...
    Param_decl_JamList: function(node) {
        this.inDecl++;
        this.declBaseType.push(node.jamtype);
        this.declSubType.push({static: false});
    ...
    Function_def_JamArray: function(node) {
        // todo
    },
    Function_def_JamList: function(node) {
        // todo
    },
    ...
    identifier: function(node) {
        if (this.inDecl == 0) {
            ...
            if (res == undefined || res == "nvoid_global")
            ...
            if (res && res.nvoid_t) {
                node.nvoid_typeinfo = res;
            }
        } else if (this.declBaseType.length > 0 && this.declSubType.length > 0) {
            let typeinfo = {nvoid_t: true,
                            jamtype: this.declBaseType.at(-1),
                            static: this.declSubType.at(-1).static};

            if (typeinfo.static)
                typeinfo.size = this.declSubType.at(-1).size;

            if (symbolTable.isScoped())
                typeinfo.type = "local_variable";
            else
                typeinfo.type = "nvoid_global"

            symbolTable.set(node.name, typeinfo);
