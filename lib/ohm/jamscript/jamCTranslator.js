...
const jamCTranslator = {
    ...
    Jcond_decl: function(node) {
        ...
        for (var decl of node.decls) {
            ...
            if (node.type === "jcond") {
                if (decl.second)
                    nds.push(astify(`int __jcond__${namesace.currentNamespace}__${decl.name.name}(jcond_my_t ${decl.first.name}, jcond_your_t ${decl.second.name}) {}`, "Function_def"));
                else
                    nds.push(astify(`int __jcond__${namesace.currentNamespace}__${decl.name.name}(jcond_my_t ${decl.first.name}) {}`, "Function_def"));
            } else {
                if (!decl.second)
                ...
                nds.push(astify(`int __jreuse__${namespace.currentNamespace}__${decl.name.name}(${decl.reuse_spec.name} ${decl.first.name}, ${decl.reuse_spec.name} ${decl.second.name}) {}`, "Function_def"));
            }
...
module.exports = {
    ...
    compile: function (ast, jCond, hasJdata, verbosity) {
        ...
        if (verbosity > 0) console.log("Generating C code...");
