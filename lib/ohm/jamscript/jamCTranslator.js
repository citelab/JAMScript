...
cTranslator.Assign_stmt_Struct = function(id, _1, _2, members, _3, _4) {
  ...
  if(symbol !== undefined && symbol.type === "jdata") {
    ...
    if(members.child(0).ctorName == "NonemptyListOf") {
      ...
      return `jamdata_log_to_server("global", "${id.sourceString}", jamdata_encode("${id.sourceString}", ${jdata.createStructCallParams(symbol.type_spec, '', assignmentMap).join(', ')}), ((void*)0));`;
    }    
  } else {
