...
const insertYieldPoints = {
    ...
    Goto_stmt: function(node) {
        if (!this.loopStack.at(-1).hasYield)
            return [astify("jyield();", "Expr_stmt"), node];
    },
...
function yieldLoop(node) {
    ...
    if (!this.loopStack.pop().hasYield)
        node.stmt.block.push(astify("jyield();", "Expr_stmt")); // TODO do we have a yield
}
...
__cTranslator.Function_def = function (specs, decl, stmts) {
	...
	var fname = decl.child(1).child(0).child(0).cTranslator; // TODO
	allFunc.push(fname);
	...
	symbolTable.enterScope();
    var declaration = decl.cTranslator;
    var params = [];
...
__cTranslator.Compound_stmt = function (_1, stmts, _2, _3) {
	...
	for (var i = 0; i < stmts.numChildren; i++) {
		...
		if (isFunctionCall(currChild) && symbolTable.getTask(currResult.slice(0, currResult.indexOf("("))) !== undefined) {
			const functionParams = currResult.slice(currResult.indexOf("(") + 1, currResult.indexOf(")")).trim();
