...
cTranslator.Assign_stmt_Struct = function(id, _1, _2, members, _3, _4) {
  ...
  if(symbol !== undefined && symbol.type === "jdata") {
    ...
    if(members.child(0).ctorName == "NonemptyListOf") {
      ...
      return `jamdata_log_to_server("global", "${id.sourceString}", jamdata_encode("${structCall.formatString}", ${structCall.valueArray.join(", ")}), ((void*)0));`;
    }
  } else {
  ...
  if(symbol !== undefined && symbol.type === "jdata") {
    ...
    if(rhs !== undefined && rhs.type === "jdata") {
      if(rhs.type_spec instanceof Object) {
        ...
        return `jamdata_decode("${res.formatString}", get_jbroadcaster_value(${expr.sourceString}), ${res.count}, &${id.sourceString}, ${res.offsetArray.join(', ')});\n`;
      }
    }
  }
