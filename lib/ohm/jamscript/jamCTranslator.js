...
const astMatcher = require("./cAstMatcher").astMatcher;
...
var jamCTranslator = {
	...
	Sync_activity: function (_1, specs, jCond_spec, _2, declarator, namespace, stmt) {
		...
		if (jCond_spec.numChildren > 0) {
			jCond = jCond_spec.jCondTranslator[0]; // TODO should be in preprocessor
		}
	...
	Async_activity: function (_1, jCond_spec, _2, decl, namespace, stmt) {
        let funcname = this._node.function_name
	...
	Source: function (decls) {
		...
		for (var i = 0; i < decls.numChildren; i++) { // TODO treewalk this...
			if (decls.child(i).child(0).ctorName === "Activity_def") {
				...
				if (decls.child(i).child(0).sourceString === "int main();") { // TODO why?
					cout = "";
					callGraph.resetCallGraph("c");
				} else {
					cout += decls.child(i).child(0).jamCTranslator + "\n";
				}
			} else if (["Jexport", "Jrequire"].indexOf(decls.child(i).child(0).ctorName) > -1) {
	...
	NonemptyListOf: function (first, sep, rest) {
		...
		for (var i = 0; i < rest.numChildren; i++) {
			code += sep.child(i).sourceString + " " + rest.child(i).jamCTranslator;
		}
		return code;
	},
...
//semantics.addOperation("pruneAst", preCompilationC.pruneAst);
semantics.addOperation("cstPrint", cstPrint);
//semantics.addAttribute("registerFunctions", preCompilationC.registerFunctions);
...
module.exports = {
	semanticAnalyze: function (input, offset) {
        ...
        fs.writeFile('cst.dot', semantics(cTree).cstPrint(), (err) => {if (err) console.error(err);});
        let ast = toAST(cTree, astMatcher);
        astTreeWalker(preCompilationC.blockifyStatements).walk(ast);
        fs.writeFile('ast.dot', printAst(ast), (err) => {if (err) console.error(err);});
		semantics(cTree).registerFunctions;
        return cTree;
	},
	compile: function (cTree, offset, manager, yieldPoint, libs, jCond, hasJdata) {
