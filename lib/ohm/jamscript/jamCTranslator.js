...
const astPrint = require("./astPrint").astPrint;
...
semantics.addOperation("astPrint", astPrint);
...
module.exports = {
	semanticAnalyze: function (input, offset) {
        ...
        semantics(cTree).pruneAst();
        fs.writeFile('ast.dot', semantics(cTree).astPrint(), (err) => {if (err) console.error(err);});
