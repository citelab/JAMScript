	...
	if (left.child(0).ctorName === "Left_expr_Member") {
		...
		if (typeof(pobj.type_spec) === 'string') {
			...
			switch (pobj.type_spec) {
				...
				break;
				case 'char*':
					return (flowfn === 'write' ? `ufwrite_str(${psrc}, ${param})` : `dfread_str(${psrc}, ${param})`);
				break;
				case 'float':
				case 'double':
...
function translate(tree, hasJ) {
	...
	let jsout = activities.generateJSActivities(symbolTable.activities.js);
	results.JS += jsout.JS;
