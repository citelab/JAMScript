...
var jamCTranslator = {
  ...
  Sync_activity: function(_, specs, jCond_spec, declarator, namespace, stmt) {
    ...
    for(var i = 0; i < params.length; i++) {
      if(params[i].type === "jcallback") {
        throw "jcallback cannot be used in synchronous activity " + funcname;
      }
      ...
      if(activity.activityType === "sync") {
        ...
        for(var i = 0; i < parameters.length; i++) {
          if(parameters[i] === "jcallback") {
            throw "jcallback cannot be used in synchronous activity " + id.cTranslator;
          }
        }
  ...
  Struct_access: function(id, _, expr) {
    ...
    // return '"' + id.sourceString.substr(1) + '", "' + expr.jamCTranslator + '"';
    return {
...
cTranslator.Dir_declarator_Id = function(id) {
  ...
  if(symbol !== undefined) {
    if(symbol.jdata_type === "broadcaster" ) {
        throw 'Cannot declare broadcaster ' + id.sourceString;
    }
  }
  ...
  if(node.ctorName === "id") {
    ...
    if(symbol !== undefined && symbol.type === "jdata") {
      if(symbol.jdata_type === "logger" || symbol.jdata_type === "shuffler") {
        ...
        return `${types.getStringCast(symbol.type_spec)}(get_jambroadcaster_value(${node.sourceString}))`;
...
cTranslator.Assign_stmt_Struct = function(id, _1, _2, members, _3, _4) {
  ...
  if(symbol !== undefined && symbol.type === "jdata") {
    ...
    if(members.child(0).ctorName == "NonemptyListOf") {
      ...
      return `jamdata_log_to_server("global", "${id.sourceString}", jamdata_encode("${structCall.formatString}", ${structCall.valueArray.join(", ")}), 1);`;
    }
  } else {
  ...
  if(symbol !== undefined && symbol.type === "jdata") {
    ...
    if(rhs !== undefined && rhs.type === "jdata") {
      if(rhs.type_spec instanceof Object) {
        ...
        return `jamdata_decode("${res.formatString}", get_jambroadcaster_value(${expr.sourceString}), ${res.count}, &${id.sourceString}, ${res.offsetArray.join(', ')});\n`;
      }
    }
  }
...
function generate_taskmain() {
  var cout = `\nvoid taskmain(int argc, char **argv) {\n
    int argoff = jamargs(argc, argv, app_id, &ndevices);
    argc = argc - argoff;
    argv = &(argv[argoff]);
    comboptr_t *cptr = create_combo3ip_ptr(NULL, NULL, NULL, argc, (void **)argv);

    js = jam_init(1883, ndevices);
    user_setup();
