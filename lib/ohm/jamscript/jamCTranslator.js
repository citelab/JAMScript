...
var jamCTranslator = {
  ...
  Sync_activity: function (_, specs, jCond_spec, declarator, namespace, stmt) {
    ...
    const cWrapper = activities.createCTaskWrapper(rtype, funcname, params);
    const functionCodes = `${rtype} ${funcname}(${params})${stmt.cTranslator}`;
  ...
  Async_activity: function (_, jCond_spec, decl, namespace, stmt) {
    ...
    const cWrapper = activities.createCTaskWrapper("void", funcname, params);
    const functionCodes = `void ${funcname}(${params})${stmt.cTranslator}`;
  ...
  Jtask_activity: function (
    ...
    const retType = returnType.jamCTranslator;
    const functionName = declarator.jamCTranslator.name;
    const functionParams = declarator.jamCTranslator.params;
    const functionStatement = statement.jamCTranslator;

    const functionWrapper = activities.createCTaskWrapper(
      retType,
      functionName,
      functionParams
    );
    ...
    const result = `${retType} ${functionName}(${functionParams})${functionStatement}\n${functionWrapper}\n`;
    return {
      C: result,
      JS: "",
      annotated_JS: "",
...
function generate_setup() {
  const functionList = Array.from(symbolTable.tasks.keys()).forEach(
    (taskName) => {
      return `register_function(cnode, ${taskName}, "", "", false);`;
    }
  );

  var cout = "\nvoid user_setup() {\n";
  cout += generateCActivities();
  cout += `${functionList !== undefined ? functionList.join("\n") : ""}`;
...
function translate(tree) {
  ...
  // TODO: Remove the following snippet after debugging.
  fs.writeFileSync("/Users/olinayu/Desktop/test.c", cout);
  fs.writeFileSync(
    "/Users/olinayu/Desktop/milestone.json",
