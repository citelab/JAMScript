...
var jamCTranslator = {
	Namespace_spec: null, // TODO handle in preprocessor
	Sync_activity: function (node) {
        ...
        let funcname = node.name.name = symbolTable.getTranslatedName(node.name.name); // TODO
	...
	Prototype: function (specs, pointer, id, _1, params, _2, gcc, _3) {
		...
		if (params.numChildren > 0) {
			let tempParams = params.jamCTranslator[0];
	...
	Pcall_decl_ParamTypeList: function (_1, node, _2) {
		let first = node.children[0].children[0].children[0].children[1]; //TODO
		let rest = node.children[0].children[2]; //TODO
		let first_id = cTranslatorFile.getDeclaratorId(first);
		if (first_id !== null) {
