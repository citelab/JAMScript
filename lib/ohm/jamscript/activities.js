...
module.exports = {
    ...
    generateMbox: function(jsActivities) {
        ...
        ${Array.from(jsActivities.keys()).map(functionName => {
            ...
            return `mbox.set("${functionName}", {arg_sig: "${signature}", side_eff: false, cond: ""});`
        }).join('\n')}
        `;
    },
    createCTaskWrapper(returnType, functionName, functionParams){
        ...
        if (returnType === 'void') {
            ...
            cOut += `${functionName}(${functionParams.map((_, index) => `t[${index}].val.sval`)});\n`;
            ...
            cOut += 'retarg.nargs = 1;\n';
            cOut += `retarg.val.sval = strdup(${functionName}(${functionParams.map((_, index) => `t[${index}].val.sval`)}));\n`;
