...
module.exports = {
    ...
    createCTaskWrapperInCESP32(returnType, functionName, functionParams){
        let cOut = '';
        if (returnType === 'void') {
            cOut += `void call_${functionName}(execution_context_t* ctx){\n`;
            cOut += 'arg_t *t = ctx->query_args;\n';
            cOut += `${functionName}(${functionParams.map((value, index) => `t[${index}].val.${types.getJamlibCode(value.type)}`)});\n`;
            cOut += '}\n'
        } 
        else {
            ...
            cOut += 'execution_context_return(ctx, &retarg);\n'
            //cOut += 'command_args_free(t);\n'
            cOut += '}\n'
        }
        return cOut;
    ...
    createJsTaskWrapperInC(returnType, functionName, functionParamTypes) {
        const functionSignature = functionParamTypes.map(type => types.getJSCode(type)).join('');
        ...
        if (returnType === 'void') {
            ...
            cOut += `bool res=remote_async_call(cnode->tboard,"${functionName}", "${functionSignature}"${functionParams.length > 0 ? ", " + functionParams.map(v => v.name).join(',') : ""});\n`;
            ...
            cOut += `arg_t* a=remote_sync_call(cnode->tboard,"${functionName}", "${functionSignature}"${functionParams.length > 0 ? ", " + functionParams.map(v => v.name).join(',') : ""});`;
