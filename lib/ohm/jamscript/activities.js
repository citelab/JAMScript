...
module.exports = {
  ...
  CreateCSyncJSFunction: function (fname, jCond, params) {
    ...
    // write the code that would call the remote function
    funccode += `return jnode.remoteSyncExec("${fname}", [ ${ps.join(',')} ], "${jCond.source}", ${jCond.code});\n`;
  ...
  CreateJSASyncCFunction: function (fname, cParams, jParams, jCond) {
    ...
    cout += `jam_rexec_async(js, jact, "${jCond.source}", ${jCond.code}, "${fname}", "${c_codes.join('')}"`;
    if (qs.length > 0) {
      cout += ',' + qs.join(', ');
    }
    cout += ');\n';
  ...
  CreateJSSyncMachFunction: function (fname, jCond, params) {
    var jsout = "function " + fname + "(" + params.join(',') + ") {\n";
    jsout += `return jnode.machSyncExec("${fname}", [ ${params.join(',')} ], "${jCond.source}", ${jCond.code});\n`;
    jsout += "}\n";
