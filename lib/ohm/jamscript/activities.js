...
module.exports = {
    CreateCASyncJSFunction: function(fname, jCond, params) {
        ...
        funccode += `jnode.remoteAsyncExec("${fname}", [ ${ps.join(',')} ], "${jCond.source}", ${jCond.code}, "${jCond.bcasts}", "${jCond.cback}");\n`;
    ...
    CreateCASyncCFunction: function(fname, params, stmt) {
        ...
        cout += 'jam_lexec_async("exec' + fname + '"' + untyped_params.join(', ') + ');\n';
        cout += '}\n';
    ...
    CreateCSyncJSFunction: function(fname, jCond, params) {
        ...
        // write the code that would call the remote function
        funccode += `return jnode.remoteSyncExec("${fname}", [ ${ps.join(',')} ], "${jCond.source}", ${jCond.code}, "${jCond.bcasts}", "${jCond.cback}");\n`;
    ...
    CreateJSASyncJSFunction: function(fname, cParams, jParams, stmt) {
        ...
        for (var i = 0; i < cParams.length; i++) {
            if (cParams[i] === "jcallback") {
                stmt = `${jParams[i]} = function(x) { jnode.remoteAsyncExecCB(_${i}, [x], "true", 0, [], ""); }\n` + stmt;
                ps.push("_" + i);
    ...
    CreateJSAsyncMachFunction: function(fname, jCond, params) {
        ...
        jsout += `jnode.machAsyncExec("${fname}", [ ${params.join(',')} ], "${jCond.source}", ${jCond.code}, "${jCond.bcasts}", "${jCond.cback}");\n`;
        jsout += "}\n";
    ...
    CreateJSSyncMachFunction: function(fname, jCond, params) {
        var jsout = "function " + fname + "(" + params.join(',') + ") {\n";
        jsout += `return jnode.machSyncExec("${fname}", [ ${params.join(',')} ], "${jCond.source}", ${jCond.code}, "${jCond.bcasts}", "${jCond.cback}");\n`;
        jsout += "}\n";
    ...
    generateMbox: function(jsActivities) {
        ...
        //jsout += 'jamlib.run(function() { console.log("JAMLib 1.0beta Initialized."); userProgram(); } );\n';
