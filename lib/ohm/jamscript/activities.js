...
module.exports = {
    ...
    generateMbox: function(jsActivities, inJStart=false) {
        ...
        ${Array.from(jsActivities.keys()).map(functionName => {
            ...
            return `mbox.set("${functionName}", {func: ${inJStart ? '"' : ''}${functionName}${inJStart ? '"' : ''}, arg_sig: "${signature}", side_eff: true, results: false, reuse: false, cond: "${jCond.tag ? jCond.tag : ''}"});`
        }).join('\n')}
        `;
    },
    ...
    createJsTaskWrapperInC(returnType, functionName, functionParamTypes) {
        ...
        if (returnType === 'void') {
            ...
            cOut += `arg_t* a=remote_sync_call(cnode->tboard,"${functionName}",${functionParams.map(v => v.name).join(',')});`;
