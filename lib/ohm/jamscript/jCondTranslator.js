...
var jCondTranslator = {
    Jcond_expr_paran: function(_1, expr, _2) {
        return {
            ...
            bcasts: expr.jCondTranslator.bcasts
        };
    },
    ...
    Jcond_expr_bin_op: function(expr1, op, expr2) {
        return {
            ...
            bcasts: expr1.jCondTranslator.bcasts
        };
    },
    Jcond_expr_namespace: function(namespace, _, id) {
        return {
            source: `jcond.get('${namespace.sourceString}.${id.sourceString}').source`,
            code: conditionTable.get(namespace.sourceString + "." + id.sourceString).code,
            cback: `jcond.get('${namespace.sourceString}.${id.sourceString}').cback`,
            bcasts: `jcond.get('${namespace.sourceString}.${id.sourceString}').bcasts`,
            tag: `${namespace.sourceString}.${id.sourceString}`
        }
        return '';

    },
    _iter: function (...children) {
        return children.map(c => c.jCondTranslator);
    },
    Jcond_expr: function(node) {
        return node.jCondTranslator;
    },
    Jcond_expr_id: function(id) {
        if (conditionTable.get(id.sourceString) == undefined) {
            throw ("jcond specification {" + id.sourceString + "} not found");
        }
        return {
            source: `jcond.get('${id.sourceString}').source`,
            code: conditionTable.get(id.sourceString).code,
            cback: `jcond.get('${id.sourceString}').cback`,
            bcasts: `jcond.get('${id.sourceString}').bcasts`,
            tag: `${id.sourceString}`
        };
    },
