...
module.exports = {
    get: function(jamtype) {
        ...
        } else if (jamtype.type === "Struct_entry") {
            if (jamtype.array)
                return jamtype.jamtype.unsigned ? types["array"]["unsigned"][jamtype.jamtype.name] : types["array"][jamtype.jamtype.name];
            return (jamtype.jamtype.unsigned ? types["unsigned"] : types)[jamtype.jamtype.name];
        }
        return types["void"];
    },
    ...
    registerStructType: function(struct_name, struct_entries) {
        ...
        for (var entry of struct_entries) {
            ...
            if (entry.array)
                jamtype = "struct " + this.registerArrayType(jamtype, entry.array);
    ...
    generateDefs: function(defs = []) {
        for (var [name, gen] of arrayTypeRegistry) {
            ...
            var e = astify(`NVOID_DEFINE_TYPE(${name}, 0, ${len});`, "Stmt");
            e.expr.args[1] = gen[0];
            defs.push(e);
        }
        for (var [name, gen] of structTypeRegistry) {
            ...
            for (var [field, type] of gen)
                entries += `${type} ${field};`;
            defs.push(astify(`struct ${name.name} {${entries}};`, "Decl"));
        }
        return defs;
    },
    ...
    generateReuseSpec: function(params) {
        let typename = `struct __jsys__jreuse__argstruct__${reuseStructCounter++}`;
        let structBody = params.map((p, i) => {
            return p.c_type_string + " arg_" + i + ";";
        });

        return {
            struct_decl: astify(`${typename} {${structBody.join("")}};`, "Decl"),
            params: params,
            typename: typename,
        };
    },
    checkReuseSpec: function(params, reusespec) {
