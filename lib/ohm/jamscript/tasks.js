...
module.exports = {
    ...
    createCTaskWrapperInC: function(returnType, functionName, functionParams) {
        ...
        let args = functionParams.map((type, index) =>
            astify(`t[${index}].val.${types.getJamlibCode(type)}`, "Expr"));
        let funcCall = astify(`${functionName}();`, "Stmt");
        ...
        if (returnType === 'void') {
            ...
            let callStmt = astify(`retarg.val.${types.getJamlibCode(returnType)} = 0;`, "Stmt");
    ...
    createJsTaskWrapperInC: function(returnType, name, functionParams) {
        ...
        const argsig = functionParams.map(type => types.getCCode(type)).join('');
        const args = functionParams.map((_, index) => `arg_${index}`).join();
        let body = [];
        let callExpr = `remote_sync_call(cnode->tboard, "${name}", "${argsig}", ${args})`;
