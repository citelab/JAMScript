...
module.exports = {
    createCTaskWrapperInC: function(ctask) {
        ...
        if (reuse || reuse_history) {
            ...
            if (reuse === "default")
                reuse += cTask.name;
            let reusedef = symbolTable.jreuse.c.get(reuse);
            ...
            for (var [index, param] of ctask.params.entries()) {
                if (param.type === "Jamparam_decl_String") {
                    ...
                    jre_cleanup.push(`free(jrn.arg_${index});`);
                } else if (param.array) {
            ...
            body.push(astify(`for (int reui = __jsys__jreuseind${ctask.name}; reui < __jsys__jreuseind${ctask.name} + __jsys__jreusecnt${ctask.name}; reui++) {
                               if (__jsys__jreuse__${reuse}(__jsys__jreusebuf${ctask.name}[reui % {reuse_history}], jrn)) {
                                 command_args_free(t);
                                 ${reuseret}
                                 return;
                               }
            ...
            body.push(astify(`if (__jsys__jreusecnt${ctask.name} == ${reuse_history}) {${jre_cleanup.join("")}} else __jsys__jreusecnt${ctask.name}++;`, "Stmt")); // free nvoids we evict from buf & resultbuf
        }
        if (!ctask.return_type) {
            ...
            if (reuse) {
                if (ctask.return_type.type === "Jamtype_return_Array")
                    body.push(astify(`__jsys__jreuseret${ctask.name}[__jsys__jreuseind${ctask.name}] = da;`, "Stmt"));
                else if (ctask.return_type.name !== "void")
                    body.push(astify(`__jsys__jreuseret${ctask.name}[__jsys__jreuseind${ctask.name}] = retarg.val.${retType.jamlib};`, "Stmt"));
            }
    ...
    createCSyncLocalWrapper: function(ctask, decls) {
        ...
        if (reuse || reuse_history) {
            ...
            let reusedef = {};
            if (reuse === "default") {
                // TODO generate default reuse function & register to the symbol table
            ...
            if (ctask.return_type) {
                ...
                if (ctask.return_type.name !== "void") {
                    reuseretval = `return retarg.val.${retType.jamlib} = __jsys__jreuseres${ctask.name}[reui % {reuse_history}];`;
                    // todo result buffer alloc
                }
                reuseret = `${jrn_cleanup.join("")}
                            ${reuseretval}`;
            }
