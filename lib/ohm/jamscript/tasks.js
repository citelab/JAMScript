...
module.exports = {
    ...
    createCSyncLocalWrapper: function(ctask, decls) {
        ...
        if (ctask.jcond) { // TODO populate the jcond structures with actual values
            body.push(astify(`jcond_my_t my;`, "Decl"));
            body.push(astify(`jcond_your_t your;`, "Decl"));

            let jcond = symbolTable.jcond.c.get(ctask.jcond.name);
            body.push(astify(`if (!${jcond.namespaced_name}(my, your)) {
                                ${properType} jcond_local_return;
                                return jcond_local_return;
                              }`, "Stmt")); // TODO how to handle?
        }
    ...
    createJsTaskWrapperInC: function(jtask) {
        let cOut;
        let body = [];
        const args = jtask.params.map((_, index) => `arg_${index}`);
        let callArgs = [`cnode->tboard`, `"${jtask.namespace_name}"`, `"${jtask.codes}"`].concat(args);
