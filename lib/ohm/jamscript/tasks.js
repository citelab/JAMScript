...
module.exports = {
    createCTaskWrapperInC: function(ctask) {
        ...
        if (reuse || reuse_history) {
            ...
            let reusedef = {};
            if (reuse === "default") {
                // TODO generate default reuse function
            ...
            if (ctask.return_type) {
                ...
                if (reuseretval) {
                    ...
                    // jre_cleanup.push(`free(__jsys__jreuseret${ctask.name}[__jsys__jreuseind${ctask.name}]);`); // actually I think we don't need to duplicate results for this buffer
                }
            ...
            body.push(astify(`for (int reui = __jsys__jreuseind${ctask.name}; reui < __jsys__jreuseind${ctask.name} + __jsys__jreusecnt${ctask.name}; reui++)
                               if (__jsys__jreuse${ctask.name}(__jsys__jreusebuf${ctask.name}[reui % {reuse_history}];, jrn)) {
                                 command_args_free(t);
                                 ${reuseret}
                                 return;
                               }`, "Stmt"));
