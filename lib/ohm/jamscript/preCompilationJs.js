...
const processManifest = {
    Source: function(node) {
        ...
        if (!this.manifest) {
            ...
            node.manifest = this.manifest;
        console.log("manny", node.manifest); // TODO
        return node;
    },
    ...
    Jfile_list: function(node) {
        ...
        console.log(node);
        this.manifest[node.file_type] = node.files;
    },
    _default: null,
};
...
function registerTask(node) {
    ...
    if (node.jcond != null) {
        if (!symbolTable.jcond.js.has(node.jcond.name))
            throw `ERROR: jcond ${node.jcond.name} undefined`;
    }
