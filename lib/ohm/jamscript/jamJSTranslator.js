...
var jamJSTranslator = {
    ...
    async function main_program() {
      await __worklib.init((x)=>{ __datalib.updateUServers(x); });
    ...
    `;
		...
		const mbox = activities.generateMbox(symbolTable.activities.js, false);
		const jstartMbox = activities.generateMbox(symbolTable.activities.js, true);

		const jcond = generateJcondMap();

		annotated_JS = requires + programInitiator + annotated_JS;

		let jdata_configR = hasJdata ? 'redserv = __datalib.createRedis(jsys.redis.host, jsys.redis.port); \n' : ''
		let jdata_creatR = hasJdata ? 'await __datalib.configureMainRedis(redserv, jsys.redis.host, jsys.redis.port, jsys.nodeid, ()=> { __worklib.notifyRedis(jsys.redis.host, jsys.redis.port); }); \n' : ''
	...
	Jdata_spec_specified: function (
		...
		if (jdata_type.jamJSTranslator === "uflow") {
			return `let ${id.sourceString} = await __datalib.createUFlow(redserv, "${id.sourceString}");`;
		} else if (jdata_type.jamJSTranslator === "dflow") {
			return `let ${id.sourceString} = await __datalib.createDFlow(redserv, '${id.sourceString}');`;
		} else if (jdata_type.jamJSTranslator === "shuffler") {
			return `let ${id.sourceString} = new JAMShuffler('${id.sourceString}', jman);\n__worklib.addJAMShuffler("${id.sourceString}", ${id.sourceString});`;
		} else {
			return;
		}
	},
	Jdata_spec_default: function (type_spec, id, _1, jdata_type, _2) {
		...
		if (jdata_type.jamJSTranslator === "uflow") {
			...
			return `let ${id.sourceString} = new JAMShuffler('${id.sourceString}', jman);\n__worklib.addShuffler("${id.sourceString}", ${id.sourceString});`;
		} else {
			return;
		}
	},
