...
var jamJSTranslator = {
        ...
        if (left.sourceString === "sys") {
            ...
            if (sentry.jdata_type === 'logger') {
                if (sentry.type_spec === 'char*') {
                    str = "lg." + left.sourceString + ".lastValue() == undefined ? undefined : lg." + left.sourceString + ".lastValue()";
                } else {
    ...
    MemberExpr: function(node) {
        ...
        if (node.ctorName === "identifier") {
            ...
            if (sentry.jdata_type === 'logger') {
                if (sentry.type_spec === 'char*') {
                    str = "lg." + node.sourceString + ".lastValue() == undefined ? undefined : lg." + node.sourceString + ".lastValue()";
                } else {
    ...
    Jconditional: function(_1, id, _2, entries, _3) {
        ...
        for (var i = 0; i < entries.numChildren; i++) {
            ...
            if (ecback == '')
                ecback = null;
            output += "jnode.jcond.set('" + namespace + entry.name + "', { source: '" + entry.string + "', code: " + entry.code + ", cback: " + ecback + ", bcasts: " + JSON.stringify(entry.bcasts) + " });\n";
            foutput += entry.func;
