...
var jamJSTranslator = {
    Program: function(directives, elements) {
        ...
        for (var i = 0; i < elements.children.length; i++) {
            if (elements.child(i).child(0).child(0).ctorName === "Activity_def") {
                ...
                hasJdata = true;
                jsout += elements.child(i).child(0).child(0).jamJSTranslator;
            } else if (elements.child(i).child(0).child(0).ctorName === "Jview_decl") {
                var out = elements.child(i).child(0).child(0).jamJSTranslator;
                jviewOut = out.jview;
                jsout += out.js;
                annotated_JS += out.js;
            } else {
        ...
        if (hasJdata) {
            ...
            requires += "var JAMShuffler = jserver.JAMShuffler;\n";
            requires += "var JAMManager = jserver.JAMManager;\n";
            requires += "var JAMBroadcaster = jserver.JAMBroadcaster;\n";
            requires += "const {Flow, ParallelFlow, PFlow, InFlow, OutFlow, RealtimeFlow} = require('flows.js')(JAMManager);\n";
        } else {
        ...
        //jsout = requires + 'function userProgram() {\n' + jsout + '\n}\n';
    ...
    Jdata_spec_specified: function(type_spec, id, _1, jdata_type, _2, level, _3, _4) {
        ...
        if (jdata_type.jamJSTranslator === 'logger') {
            return `var ${id.sourceString} = new JAMLogger(JAMManager, "${id.sourceString}");\njnode.addLogger("${id.sourceString}", ${id.sourceString}.getMyDataStream());`;
        } else if (jdata_type.jamJSTranslator === 'broadcaster') {
            return `var ${id.sourceString} = new JAMBroadcaster('${id.sourceString}', JAMManager);\njnode.addBroadcaster("${id.sourceString}", ${id.sourceString});`;
        } else if (jdata_type.jamJSTranslator === 'shuffler') {
            return `var ${id.sourceString} = new JAMShuffler('${id.sourceString}', JAMManager);\njnode.addJAMShuffler("${id.sourceString}", ${id.sourceString});`;
        } else {
            return;
        }
    },
    Jdata_spec_default: function(type_spec, id, _1, jdata_type, _2) {
        ...
        if (jdata_type.jamJSTranslator === 'logger') {
            ...
            return `var ${id.sourceString} = new JAMShuffler('${id.sourceString}', JAMManager);\njnode.addShuffler("${id.sourceString}", ${id.sourceString});`;
        } else {
            return;
        }
    },
    ...
    Jview_decl: function(_1, _2, specs, _3) {
        var output = {};
        output.package = {};
        output.package.name = "JView";
        output.package.version = "1.0.0";
        output.package.description = "JView";
        output.package.author = "";
        output.port = port;
        output.maxMemorySize = 30;

        // output.pages = specs.jamJSTranslator;
        var out = specs.jamJSTranslator;
        var jsout = "";
        var outputPages = [];
        for (let i = 0; i < out.length; i++) {

            jsout += out[i].js;
            outputPages.push(out[i].jview);
        }
        output.pages = outputPages;
        return {
            js: (function () {
                if (port)
                    return "var io = require('socket.io-client');\nvar socket = io('http://localhost:" + port + "');\n" + jsout;
                else
                    return jsout;
            })(),
            jview: output
    ...
    Jview_spec_beat: function(_1, _2, id, _3) {
        //TODO
        return "";
    },
    Jview_spec: function(pages) {
        return pages.jamJSTranslator;
    },
    Jview_page: function(id, _1, _2, _3, elems, _4) {
        var output = {};
        output.name = id.sourceString;
        output.route = "/" + id.sourceString;
        output.layout = "default";

        var outputPanels = [];
        var jsout = "";
        for (let i = 0; i < elems.jamJSTranslator.length; i++) {
            jsout += elems.jamJSTranslator[i].js;
            outputPanels.push(elems.jamJSTranslator[i].jview);
        }
        output.panels = outputPanels;
        return {
            jview: output,
            js: jsout
        };
    ...
    Jconditional: function(_1, id, _2, entries, _3) {
        ...
        for (var i = 0; i < entries.numChildren; i++) {
            ...
            if (ecback == '')
                ecback = null;
            output += "jnode.jcond.set('" + namespace + entry.name + "', { source: 'eval(" + entry.string + ")', code: " + entry.code + ", cback: " + ecback + ", bcasts: " + JSON.stringify(entry.bcasts) + " });\n";
            foutput += entry.func;
...
function generateFlowDecl(lsrc, flname, rsrc) {
    ...
    src += '\n';
    src += "jnode.addFlow('" + flname + "', " + flname + ");\n";
