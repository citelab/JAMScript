...
//    jsout += "var jcondition = new Map();\n";
  ...
  MemberExpr_propRefExp: function(left, op, right) {
    ...
    if (left.sourceString === "sys")
        ...
        return left.sourceString + ".getValues()" + "." + right.sourceString;
    else
        return left.jamJSTranslator + "." + right.sourceString;
  },
  MemberExpr: function(node) {

    if (node.ctorName === "identifier") {
        return node.sourceString + ".getValues()";
    } else
        return node.jamJSTranslator;
  },
  Jcond_rule: function(left, op, right) {
    ...
    return {
      string: "jcondContext(\"" + left.jamJSTranslator + "\") " + op.sourceString + ' ' + right.jamJSTranslator,
      code: code
    };
  },
  ...
  Jconditional: function(_1, id, _2, entries, _3) {
    ...
    for(var i = 0; i < entries.numChildren; i++) {
      ...
      output += "jnode.jcond.set('" + namespace + entry.name + "', { source: '" + entry.string + "', code: " + entry.code + " });\n";
