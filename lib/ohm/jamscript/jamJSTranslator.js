...
var jamJSTranslator = {
	...
	Sync_activity: function (_1, jCond_spec, _2, functionDeclaration) {
		...
		var x = functionDeclaration.jamJSTranslator;
		var code = x.code;
		var specs = x.specs;
	...
	Async_activity: function (_1, jcond_spec, _2, functionDeclaration) {
		...
		const code = "async " + functionDeclaration.es5Translator + "\n";
	...
	SYFunctionDeclaration: function (_1, id, _2, params, _3, _4, body, _5) {
		tableManager.enterScope();
		...
		for (var node of body.child(1).children) {
			...
			if (isReturnStatement(node))
				console.log("Return statement found ", node.es5Translator);
		...
		var result = `\n async function ${id.es5Translator}(${
		params.es5Translator
	//	}) {\n return new Promise((resolve, reject)=> { ${functionBodyResults.join("\n")} });}\n`;
		...
		return {specs: specs, code: result};
	}
};
...
function isReturnStatement(node) {
	return (
		node.child(0).ctorName === "Statement" &&
		node.child(0).child(0).ctorName === 'ReturnStatement'
	);
}
