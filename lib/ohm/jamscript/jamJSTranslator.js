...
module.exports = {
  translate: function(tree) {
    ...
    output.jsActivities = jsActivities;
    output.jsFunctions = jsFunctions;

    var results = activities.generateJSActivities(jsActivities);
    output.JS += results.JS;
    output.annotated_JS += results.annotated_JS;

    output.JS += generate_js_mbox();
    output.annotated_JS += generate_js_mbox();
