...
const preCompilationC = require("./preCompilationJS");
...
const jamJSTranslator = {
	Program: function (directives, elements) {
		...
		for (var lib of exportLibs)
			if (lib.side === "J" && allFunc.indexOf(lib.function_name) < 0)
				throw ("Function " + lib.function_name + " cannot be exported because it does not exist on JS side.");

		let requires = '';
		if (hasJdata)
			requires = `const __worklib = require('modules/jworklib');
		...
		const programInitiator = `async function main_program() {
         await __worklib.init((x)=>{ __datalib.updateUServers(x, jsys.id, jsys.app); });
         jsys = __worklib.getjsys();
         await last_init();
         await user_program();
       }
       main_program().then(()=> {console.log("Program completed.");}).catch((e)=> {console.error("Program error! ", e);})`;

		const jamoutConditionAndMBoxRegistration = `__worklib.registerFuncs(mbox);
		...
		const jsOut = requires + programInitiator + 'async function user_program(){ \n' + jdata_configR + jdata_getid + jdata_getappid + jdata_creatR + mbox + jcond + jamoutConditionAndMBoxRegistration + jsout + '}\nasync function last_init() {\n  jsys.setMilestoneCount(' + milestone.getMilestoneCount() + '); \n}\n';
	...
	_nonterminal: function (...children) {
		...
		var childResults = flatChildren.map((n) => n.jamJSTranslator);
...
__es5Translator.AssignmentStatement_expression = function (left, _2, right, _4) {
	...
	if (isFunctionCall(right)) {
		...
		if (symbolTable.getTask(functionName) !== undefined) {
			const functionParams = rightString.slice(rightString.indexOf("(") + 1, rightString.indexOf(")")).trim();
			const functionInfo = symbolTable.getTask(functionName);
			if (functionInfo.language === "c")
				rightString = rightString.replace(/[a-zA-Z0-9]+\(.*\)/g, `${functionInfo.taskType === "async" ? "__worklib.remoteExecNoRV" : "__worklib.remoteExecRV"}("${functionName}", "x" ${functionParams.length > 0 ? ", " + functionParams : ""})`);
			else if (functionInfo.language === "j")
				rightString = rightString.replace(/[a-zA-Z0-9]+\(.*\)/g, `${functionInfo.taskType === "async" ? "local_jtask_nv" : "local_jtask"}(cnode, "${functionName}${functionParams.length > 0 ? ", " + functionParams : ""}")`);
		}
	}
...
__es5Translator.CallExpression_memberExpExp = function (exp, args) {
	...
	if (exp.sourceString === "require") {
		...
		if (importLibs.indexOf(namespace) > -1) {
			...
			var result = "CreateLibExec(" + '"' + namespace + '", "' + opr + '" , "' + par + '")';
...
__es5Translator.CallExpression_callExpExp = function (exp, args) {
	if (exp.sourceString === "require") {
		var moduleName = args.child(1).sourceString.slice(1, -1);
		try {
			require.resolve(moduleName, {paths: ["~/.jamruns"]});
...
function isSelectiveStatement(node) {
	return node.child(0).ctorName === "Statement" && node.child(0).child(0).ctorName === "IfStatement"
}

function isIterativeStatement(node) {
	return node.child(0).ctorName === "Statement" && node.child(0).child(0).ctorName === "IterationStatement";
}
...
function isFunctionCall(node) {
	...
	return code.match(declarationPattern) != null || code.match(statementPattern) != null;
}
...
function generateJcondMap() {
	return `const conds = new Map();
           ${Array.from(jCondMap.keys()).map((jCondTag) =>`conds.set("${jCondTag}", {source: \`${jCondMap.get(jCondTag)}\`});`).join("\n")}`;
}

__es5Translator.FunctionDeclaration = function (_1, id, _2, params, _3, _4, body, _5) {
...
__es5Translator.FunctionExpression_named = function (_1, id, _2, params, _3, _4, body, _5) {
	currentFunction = id.es5Translator;
	allFunc.push(id.es5Translator);
	symbolTable.addFunction(currentFunction, "js");
	callGraph.addFunction("js", currentFunction);
...
__es5Translator.FunctionExpression_anonymous = function (_1, _2, params, _3, _4, body, _5) {
...
__es5Translator.VariableDeclaration = function (id, initialiser) {
	id = id.es5Translator;
	if (initialiser.child(0) !== undefined) {
		var init = initialiser.child(0).child(1);
		if (init.ctorName === "ConditionalExpression" && isAnonymousFuncExpr(init))
			allFunc.push(id);
	}
...
function isAnonymousFuncExpr(condExpr) {
	...
	while (node.ctorName !== undefined) {
		if (node.ctorName === "FunctionExpression")
			return node.child(0).ctorName === "FunctionExpression_anonymous"
		node = node.child(0);
	}
	return false;
}
...
module.exports = {
	...
	compile: function (ast, yieldPoint) {
		//exportLibs = libs;
		if (yieldPoint) { // TODO tatata
