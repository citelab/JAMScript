    ...
    Jcond_decl: function(node) {
        ...
        for (var decl of node.decls) {
            if (node.jtype === "jreuse") {
                nds.push(astify(`jsys.reusefunctions.${decl.name.name} = function(${decl.first.name}, ${decl.second.name}) {};`, "Statement"));
...
function generateTask(node) {
    ...
    if (node.jtask_attr.reuse || node.jtask_attr.reuse_history) {
        ...
        reusebody.push(astify(`let __jsys__jrn = new Map(arguments.map((e, i) => ["arg_" + i, e]));`, "SourceElement"));
        ...
        reusebody.push(astify(`let __jsys__retval = __jsys__taskfunc(${node.params.map((p) => p.name.name)});`, "SourceElement"));
...
module.exports = {
    ...
    compile: function (ast, verbosity) {
        ...
        if (verbosity) console.log("Generating JavaScript Code...");
