...
const jamJsTranslator = {
    ...
    Sync_task: function(node) {
        this.inSyncTask.push(true);
        var body = astify("{return new Promise((resolve, reject)=>{});}", "FunctionBody");
        body.content[0].value.args[0].body = this.walk(node.body);
        this.inSyncTask.pop();
        return {
    ...
    ReturnStatement: function(node) {
        if (this.inSyncTask.length > 0 && this.inSyncTask.at(-1))
            return {
                type: "ExpressionStatement",
                expr: {
                    type: "FuncallExpr",
                    name: "resolve",
                    args: [this.walk(node.value)]
                }
            };
