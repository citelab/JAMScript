...
const jamJsTranslator = {
	Program: function (node) {
        ...
        newContent.push(astify(`const conds = new Map();`, "Statement"));
        for (var {jCondTag, jcsrc} of jCondMap)
            newContent.push(astify(`conds.set("${jCondTag}", {source: \`${jcsrc}\`});`, "Statement"));
        newContent.push(astify(`__worklib.registerConds(conds);`, "Statement"));

        newContent = newContent.concat(tasks.generateMbox(symbolTable.tasks.js, false));
        newContent.push(astify(`__worklib.registerFuncs(mbox);`, "Statement"));
