...
var jamCTranslator = {
  Sync_activity: function (_, specs, jCond_spec, declarator, namespace, stmt) {
    ...
    if (VERBOSE) {
      console.log(`ASYNC FUNCTION [C] --> NAME: ${funcname}`);
    }
    milestone.registerFunction(funcname, "SYNC");
  ...
  Async_activity: function (_, jCond_spec, decl, namespace, stmt) {
    ...
    milestone.registerFunction(funcname, "ASYNC");
  ...
  Prototype: function (specs, pointer, id, _1, params, _2, gcc, _3) {
    ...
    if (symbolTable.activities.js.has(id.cTranslator)) {
      ...
      if (VERBOSE) {
        console.log(`REGULAR FUNCTION [C] --> NAME: ${id.cTranslator}`);
      }
      milestone.registerFunction(id.cTranslator, "BATCH");
  ...
  Jtask_activity: function (
    ...
    const functionName =
      declarator.children[1].children[0].children[0].cTranslator;
    symbolTable.addTask(functionName, 'c', {});
    return {
      c: '',
      js: '',
      annotated_JS: ''
...
cTranslator.Function_def = function (specs, decl, stmts) {
  ...
  if (VERBOSE) {
    console.log(`REGULAR FUNCTION [C] --> NAME: ${fname}`);
  }
  milestone.registerFunction(fname, "BATCH");
  allFunc.push(fname);
