        ...
        if (jdata.type === "Jdata_spec_Basic" || jdata.type === "Jdata_spec_Array") {
            ...
            for (var entry of jdata.struct_entries) {
                ...
                if (entry.array) {
                    ...
                    func.body.block.push(astify(`value->${entry.name.name}.maxlen = ${parseInt(entry.array)};`, "Stmt"));
                    func.body.block.push(astify(`value->${entry.name.name}.size = NVOID_ALIGNED_SIZE(NULL, ${parseInt(entry.array)});`, "Stmt"));
                    func.body.block.at(-1).expr.rhs.args[0] = types.stringifyType(entry.jamtype);
                    func.body.block.push(astify(`value->${entry.name.name}.typesize = sizeof(${types.stringifyType(entry.jamtype)});`, "Stmt"));
                    func.body.block.push(astify(`value->${entry.name.name}.typefmt = '${types.get(entry.jamtype).c_code}';`, "Stmt"));
                }
                callArgs.push(astify(`&(value->${entry.name.name})`, "Expr"));
            }
