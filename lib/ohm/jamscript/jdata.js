...
module.exports = {
    ...
    createFormatAndArrayForRead: function(param, elements) {
        let result = {
            formatString: "",
            valueArray: []
        };
...
function uflowWriteWrapper(jdata) {
    ...
    if (jdata.type === "Jdata_spec_Basic") {
        ...
        for (var entry of jdata.struct_entries) {
            ...
            if (entry.array) {
                codes += "n";
                var tc = types.get(entry.jamtype).jamlib;
                ...
                codes += types.get(entry.jamtype).jamlib;
                callArgs.push(astify(`value->${entry.name.name}`, "Expr"));
            }
        ...
        var tc = types.get(jdata.jamtype).jamlib;
