  ...
  createJdataCall: function (namespace, id, value, type, spec) {
    if (type === "broadcaster") {
      ...
      if (spec === "char" || spec === "char*") {
        return `jamdata_log_to_server("${namespace}", "${id}", ${value});`;
      } else {
        var cout = `nvoid_t *jdbuf = jamdata_encode("${types.getFormatSpecifier(spec)}", ${value});\n`;
        cout += `jamdata_log_to_server("${namespace}", "${id}", jdbuf);`;
        return cout;
      }
    }
  },
