...
module.exports = {
  ...
  registerFunctionsForMilestone(milestoneId, functions) {
    ...
    if (milestoneFunctionTable.hasOwnProperty(milestoneId))
      milestoneFunctionTable[milestoneId] =
        milestoneFunctionTable[milestoneId].concat(functions);
    else milestoneFunctionTable[milestoneId] = functions;
  },
  /**
  ...
  getJCodeToEmitMilestone: function (milestone, commentOut = true) {
    return `${commentOut ? "//" : ""}milestone_log(${milestone});\n`;
  },
