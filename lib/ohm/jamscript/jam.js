...
//     signatures.c.sync.set(key, c_codes);
//   });
//   return cout;
// }
...
function generate_js_signatures() {
  var jsout = 'var mbox = {\n';
  jsout += '"functions": {\n';
  for(const [name, signature] of signatures.js.sync) {
    jsout += `"${name}": call${name},\n`;
  }
  for(const [name, signature] of signatures.js.async) {
    jsout += `"${name}": call${name},\n`;
  }
  jsout += '},\n';
  jsout += '"signatures": {\n';
  for(const [name, signature] of signatures.js.sync) {
    if(typeof(signature) === 'number') {
      jsout += `"${name}": ${signature},\n`;
    } else {
      jsout += `"${name}": "${signature.join('')}",\n`;
    }
  }
  for(const [name, signature] of signatures.js.async) {
    if(typeof(signature) === 'number') {
...
function generate_c_activities() {
  var cout = '';
  for(const [name, signature] of signatures.c.sync) {
    cout += 'activity_regcallback(js->atable, "' + name + '", SYNC, "' + signature.join('') + '", call' + name + ');\n';
  }
  for(const [name, signature] of signatures.c.async) {
    cout += 'activity_regcallback(js->atable, "' + name + '", ASYNC, "' + signature.join('') + '", call' + name + ');\n';
  }
  return cout;
}
...
function CreateCASyncCFunction(dspec, fname, params, stmt) {
  ...
  signatures.c.async.set(fname, c_codes);
  return cout;
}
...
function CreateCSyncCFunction(dspec, fname, params, stmt) {
  ...
  signatures.c.sync.set(fname, c_codes);
  return cout;
}
...
function CreateJSASyncJSFunction(fname, cParams, jParams, stmt) {
  ...
  signatures.js.async.set(fname, js_codes);
  return {
    JS: jsout, 
...
function CreateJSSyncJSFunction(rtype, fname, cParams, jParams, stmt) {
  ...
  signatures.js.sync.set(fname, js_codes);
  return {
    JS: jsout, 
    annotated_JS: annotated_jsout
  };
...
function CreateJSAsyncMachFunction(fname, jCond, params) {
  ...
  signatures.js.async.set(fname, params.length);
...
function CreateJSSyncMachFunction(fname, jCond, params) {
  ...
  signatures.js.sync.set(fname, params.length);
...
var jamCTranslator = {
  ...
  Source: function(decls) {
    ...
    jsout += generate_js_signatures();
    annotated_JS += generate_js_signatures();
  ...
  if(prototypes.has(left.cTranslator)) {
    ...
    for (var i = 0; i < prototypeParams.length; i++) {
      if(prototypeParams[i] == "jcallback") {
        ...
        signatures.c.async.set(jcallbackName, ["s"]);
...
es5Translator.FunctionDeclaration = function(_1, id , _2, params, _3, _4, body, _5) {
  ...
  if(params.jamJSTranslator.length == 1) {
    signatures.js.sync.set(currentFunction, ["s"]);
  }
