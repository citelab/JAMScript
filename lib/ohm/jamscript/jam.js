...
function CreateCASyncJSFunction(fname, params) {
  ...
  funccode += 'jnode.remoteAsyncExec("' + fname + '", [' + ps.join(',') + '], "true", 0);\n';
...
function CreateJSASyncCFunction(fname, cParams, jParams) {
  ...
  cout += 'jactivity_t *res = jam_rexec_async(js, jact, \"true\", 0, "' + fname + '", "' + c_codes.join('') + '"';
  if(qs.length > 0) {
    cout += ',' + qs.join(', ');
  }
  cout += ');\n';
...
function CreateJSSyncCFunction(dspec, fname, cParams, jParams) {
  ...
  cout += 'arg_t *res = jam_rexec_sync(js, \"true\", 0, "' + fname + '", "' + c_codes.join('') + '"';
  if(qs.length > 0) {
    cout += ',' + qs.join(', ');
  }
  cout += ');\n';
...
var jamJSTranslator = {  
  ...
  Jcond_rule: function(id, op, lit) {
    return 'jcondition_context["' + id.sourceString + '"] ' + op.sourceString + ' ' + lit.es5Translator;
  },
  Jcond_entry: function(id, _1, rules, _2) {
    return rules.jamJSTranslator;
  },
  Jconditional: function(_1, id, _2, entries, _3) {
    if(id.numChildren > 0) {
      return "jcondition.set('" + id.es5Translator + "', '" + entries.jamJSTranslator + "');\n";
    } else {
      return "jcondition.set('default', '" + entries.jamJSTranslator + "');\n";
    }
  },
