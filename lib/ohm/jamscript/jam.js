...
var jamJSTranslator = {  
  ...
  Sync_activity: function(_, functionDeclaration) {
    ...
    // activities.js.sync.push({
    //   name: specs.fname,
    //   params: specs.params
    // });
    // console.log(prototypes.keys());
    var js_output = CreateJSSyncJSFunction(rtype, specs.fname, cParams, jParams, specs.block);
  ...
  Async_activity: function(_, functionDeclaration) {
    var specs = functionDeclaration.jamJSTranslator;
    // activities.js.sync.push({
    //   name: specs.fname,
    //   params: specs.params
    ...
    var js_output = CreateJSASyncJSFunction(specs.fname, cParams, jParams, specs.block);
...
var jamCTranslator = {
  ...
  Async_activity: function(_, decl, namespace, stmt) {
    ...
    var c_output = CreateCASyncCFunction("jactivity", funcname, params, stmt.cTranslator);
    return {
      C: c_output,
      JS: js_output.JS,
      annotated_JS: js_output.annotated_JS
  ...
  Source: function(decls) {
    ...
    for (var i = 0; i < decls.numChildren; i++) {
      ...
      if(decls.child(i).child(0).ctorName == "Activity_def") {
        ...
        decls.child(i).child(0).jamCTranslator;
      } else {
