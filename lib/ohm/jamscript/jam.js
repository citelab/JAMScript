...
var jamJSTranslator = {  
  ...
  Jcond_specifier: function(_1, jconds, _2) {
    var jcondArray = [];
    var jsout = '';
    if(jconds.child(0).ctorName == "NonemptyListOf") {
      var list = jconds.child(0);
      jcondArray.push(list.child(0).es5Translator);
      var rest = list.child(2);
      for (var i = 0; i < rest.numChildren; i++) {
        jcondArray.push(rest.child(i).es5Translator);
      }
    }
    jsout += 'var jcondition_context = jlib.JServer.get_jcond();\n'
    jsout += 'if(!(';
    jsout += "eval(jcondition.get('" + jcondArray[0] + "'))";
    for (var i = 1; i < jcondArray.length; i++) {
      jsout += " && eval(jcondition.get('" + jcondArray[i] + "'))"
    }
    jsout += ')){\n';
