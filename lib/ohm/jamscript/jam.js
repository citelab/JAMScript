...
var ohm = require('/usr/local/share/jam/deps/ohm');
...
var prettyish = {
  ...
  Program: function(decls) {
    ...
    for (var i = 0; i < decls.numChildren; i++) {
      if(decls.child(i).child(0).ctorName == "Activity_def") {
        var output = decls.child(i).child(0).prettyish;
        cout += output.C + '\n';
        jsout += output.JS + '\n';
  ...
  Async_decl: function(_, declarator, namespace) {
    var funcname = declarator.prettyish.name;
    var params   = declarator.prettyish.params;
  ...
  Sync_decl: function(_, decl_specs, declarator, namespace) {
    ...
    if(namespace.numChildren > 0) {
        // funcname = namespace.cTranslator + "_func_" + namespace_funcs[namespc].indexOf(funcname);
    }
  ...
  Param_type_lst: function(param_list) {
    var params = []
    params.push(param_list.child(0).prettyish);
    var rest = param_list.child(2);
    for (var i = 0; i < rest.numChildren; i++) {
      params.push(rest.child(i).prettyish);
    }
    return params;
  },
  Declarator: function(pointer, dir_declarator, _1, _2) {
    var dir_decl = dir_declarator.prettyish;
    return {
  ...
  Param_decl_Declarator: function(decl_specs, decl) {
    ...
    if(decl.prettyish.pointer != '') {
      varType += ' ' + decl.prettyish.pointer
    }
    return {
      type: varType,
      name: decl.prettyish.name 
    };
  },
  C_as_activity: function(decl, stmt) {
    var specs = decl.prettyish;
  ...
  C_s_activity: function(decl, stmt) {
    ...
    var c_output = CreateCSyncCFunction(specs.fname, specs.params, stmt.cTranslator);
    return {
      C: c_output,
      JS: js_output.JS,
      annotated_JS: js_output.annotated_JS
