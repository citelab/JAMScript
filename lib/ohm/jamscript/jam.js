...
function CreateJSASyncJSFunction(fname, cParams, jParams, stmt) {
  ...
  var jsout = "var " + fname + " = " + "function(" + ps.join(',') + ") {\n" + stmt + "\n};";
  var annotated_jsout = "var " + fname + " = " + "function(" + annotated_ps.join(',') + "): void {\n" + stmt + "\n};";
...
function CreateJSSyncJSFunction(rtype, fname, cParams, jParams, stmt) {
  ...
  var jsout = "var " + fname + " = function(" + ps.join(',') + ") {\n" + stmt + "\n};";
  var annotated_jsout = "var " + fname + " = function(" + annotated_ps.join(',') + "):" + js_return_type + " {\n" + stmt + "\n};";
...
cTranslator.Function_def = function(specs, decl, stmts) {
    ...
    if(decl.cTranslator == "main()") {
      ...
      // Get function params
      var params = decl.child(1).child(0).child(1).child(0).child(1).cTranslator.split(', ');
