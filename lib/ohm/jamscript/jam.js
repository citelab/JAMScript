...
function CreateJSAsyncMachFunction(fname, jCond, params) {
  ...
  for(var i = 0; i < ps.length; i++) {
    jsout += `if(typeof ${ps[i]} === "function") { ${ps[i]} = ${ps[i]}.name; }\n`;
  }

  jsout += `jnode.machAsyncExec("${fname}", [ ${ps.join(',')} ], "${jCond.source}", ${jCond.code});\n`;
  jsout += "}\n";
...
function CreateJSSyncMachFunction(fname, jCond, params) {
  ...
  var jsout = "function " + fname + "(" + ps.join(',') + ") {\n";
  jsout += `yield wait.for(jnode.machSyncExec("${fname}", [ ${ps.join(',')} ], "${jCond.source}", ${jCond.code}));\n`;
  jsout += "}\n";
