...
function CreateJSASyncJSFunction(fname, cParams, jParams, stmt) {
  ...
  for (var i = 0; i < cParams.length; i++) {
    if(cParams[i] == "jcallback") {
      stmt = `${jParams[i]} = function(x) { jnode.remoteAsyncExec(_${i}, [x], "true"); }\n` + stmt;
      var jcallbackCode = "";
...
var jamJSTranslator = {  
  ...
  Jdata_spec: function(type_spec, id, _1, jdata_type, _2) {
    return {
      ...
      type_spec: jdata_type.sourceString,
...
var jamCTranslator = {
  ...
  Sync_activity: function(_, specs, declarator, namespace, stmt) {
...
cTranslator.Left_expr_Call = function(left, params) {
  ...
  if(prototypes.has(left.cTranslator)) {
    ...
    paramString = '(' + paramArray.join(', ') + ')';
      ...
      cout += 'jactivity_t *res = jam_rexec_async(js, jact, ' + left.cTranslator 
...
function createJdataCall(id, value, type) {
  if(type == "broadcaster" ) {
...
cTranslator.Assign_stmt_named = function(namespace, _1, id, _2, expr, _3) {
  ...
  if(symbol !== undefined && symbol.type == "jdata namespace") {
    if(symbol.other.children.has(id.sourceString)) {
      console.log("HER");
      return createJdataCall(namespace.sourceString + '.' + id.sourceString, expr.cTranslator, symbol.other.children.get(id.sourceString).jdata_type);
    }
  }
...
cTranslator.Assign_stmt_anonymous = function(id, _1, expr, _2) {
  var symbol = symbolTable.get(id.sourceString);
  if(symbol !== undefined && symbol.type === "jdata") {
    return createJdataCall(id.sourceString, expr.cTranslator, symbol.other.jdata_type);
  }
