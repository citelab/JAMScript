...
const cTranslator = {
    ...
    Initializer_DesignatorList: function(node) {
        return noneNode(node.desigs) ? "{}" : "{" + this.walk(node.desigs).join(", ") + "}";
    },
    Designator_Struct: function(node) {
        return "." + node.field + this.walk(node.desig);
    },
    Designator_Array: function(node) {
        let end = nodeNode(node.end) ? "" : " ... " + this.walk(node.end);
        return "[" + this.walk(node.start) + end + "]" + this.walk(node.desig);
    },
    Desig_assign: function(node) {
        return " = " + this.walk(node.expr);
    },
    // TODO gcc stuffs
    ...
    For_stmt: function(node) { // this one is kind of ugly for formatting reasons
        this.tabd++;
        let expr_init = this.walk(node.expr_init);
        let cond = noneNode(node.cond.expr) ? ";" : " " + this.walk(node.cond.expr) + ";";
        let iter = noneNode(node.expr_iter) ? "" : " " + this.walk(node.expr_iter);
...
var __cTranslator = {
  Assign_expr_assign: function(left, op, right) { // TODO pass to collect side effects
  ...
  Assign_stmt_Struct: function(left, _1, _2, right, _3, _4) { // TODO pass for sideeffect checking
    tableManager.setHasSideEffect(left.cTranslator);
    return left.cTranslator + ' = {' + right.cTranslator + '};';
  },
  Assign_stmt_Expr: function(left, _1, right, _2) { // TODO pass for sideeffect checking
    tableManager.setHasSideEffect(left.cTranslator);
    return left.cTranslator + ' = ' + right.cTranslator + ';';
  },
  ...
  Jump_stmt_GotoStmt: function(_1, id, _2) { // TODO yield points for this ??? could be a loop implicitly
