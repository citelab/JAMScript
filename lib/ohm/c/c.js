...
const cTranslator = {
    ...
    Prototype: function(node) { // TODO check what happens with `extern`
        ...
        console.log(this.walk(node.params));
        let params = noneNode(node.params) ? "()" : "(" + this.walk(node.params).join(", ") + ")";
    ...
    Type_spec_Modified: function(node) {
        let type_name = noneNode(node.type_name) ? "" : " " + this.walk(node.type_name);
    ...
    Compound_stmt: function(node) {
        this.tabc++;
        let block = noneNode(node.block) ? "{}" : "{" + this.walk(node.block) + this.tabr(-1) + "}";
        this.tabc--;
        return block;
    },
    ...
    Function_def: function(node) {
        return this.walk(node.return_type) + this.walk(node.decl) + " " + this.walk(node.body);
    },
};
