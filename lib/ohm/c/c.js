...
const VarTable = require('../jamscript/VarTable');
var currentTable = new VarTable(null);
...
function insertYieldPoint(stmt) {
    if (stmt._node.children[0].ctorName === 'Compound_stmt') {
...
function isDeclaratorId(decl) {
  var children = decl._node.children;
  var isId;
  for (var child of children) {
    if (child.ctorName === "_iter" && child.children.length > 0) {
      isId = false;
      break;
    } else if (child.ctorName === "Dir_declarator") {
      if (child.children[0].ctorName === "Dir_declarator_Id") {
        isId = true;
      } else {
        isId = false;
        break;
...
function getPrimaryExpr(unaryExpr) {
  var isPrimary =  unaryExpr.children[0].ctorName === "Postfix_expr" && 
                  unaryExpr.children[0].children[0].ctorName === "Left_expr" && 
                  unaryExpr.children[0].children[0].children[0].ctorName === "Primary_expr";

  if (isPrimary) {
    return unaryExpr.children[0].children[0].children[0];
  } else {
    return null;
  }
}
...
var cTranslator = {
  ...
  Assign_expr_assign: function(left, op, right) {
    ...
    if (leftSide !== null) {
      if (leftSide.children[0].ctorName === "Primary_expr_GroupExpr") {
        ...
        hasSideEffect || currentTable.isVarGlobal(leftSide);
      } else if (leftSide.children[0].ctorName === "id") {
        hasSideEffect = hasSideEffect || currentTable.isVarGlobal(leftSide.cTranslator);
      }
    }
  ...
  Init_decl_Init:function(decl, _, init) {
    if (isDeclaratorId(decl)) {
      var idName = decl.cTranslator;
      currentTable.addVar(idName);
    }
  ...
  Struct_spec_Full: function(_1, _2, id, _3, decls, _4) {
    currentTable.addVar(id.cTranslator);
  ...
  Assign_stmt_Struct: function(left, _1, _2, right, _3, _4) {
    hasSideEffect = hasSideEffect || currentTable.isVarGlobal(left.cTranslator);
  ...
  Pcall_decl_ParamTypeList: function(_1, node, _2) {
    ...
    if (isDeclaratorId(first)) {
      currentTable.addVar(first.cTranslator);
    }
    ...
    if (rest.children.length > 0) {
      ...
      for (var child of children) {
        ...
        if (isDeclaratorId(child_decl)) {
          currentTable.addVar(child_decl.cTranslator);
        }
      }
    }
