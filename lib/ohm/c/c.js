...
//   if (expr.ctorName === "Assign_expr_assign") {
//     return true;
//   }
...
//       var c;
//       var result = true;
//       if (expr.ctorName === "Lar_expr_lar" || expr.ctorName === "And_expr_and") {
//         for (c of children) {
//           if (c != undefined && c.ctorName != "_terminal" ) {
//             result = result && evalExpr(c);
//           }
//         }
//       } else if (expr.ctorName === "Lor_expr_lor") {
//         for (c of children) {
//           if (c != undefined && c.ctorName != "_terminal" ) {
//             result = result || evalExpr(c);
//           } 
...
function insertYieldPoint(expr, stmt){
    expr = expr.cTranslator;
    var hasVar = false;
    ...
    if (!hasVar && expr) {
      // Check if the stmt is a compound stmt
...
var cTranslator = {
  ...
  Iteration_stmt_WhileStmt: function(_1, _2, expr, _3, stmt) {
    var newStmt = insertYieldPoint(expr, stmt);
    return `while(${newStmt.expr}) ${newStmt.stmt}`;
  },
  Iteration_stmt_DoWhileStmt: function(_1, stmt, _2, _3, expr, _4, _5) {
    var newStmt = insertYieldPoint(expr, stmt);
    return `do ${newStmt.stmt} while(${newStmt.expr});`;
  },
  Iteration_stmt_ForStmt: function(_1, _2, first, second, third, _3, stmt) {
    // TODO: Insert yield point to an infinite loop
    // But infinite for loops like for (;;) has a parsing error thrown by our compiler
