...
var cSymbolTable = {
  Decl: function(specs, list, _) {
    console.log(this.sourceString);
    console.log(specs.cTranslator);
    console.log(list.cTranslator);
    // return {
    //   name: decl.cTranslator,
    //   type: init.cTranslator
    // }
  },
  _nonterminal: function(children) {
    for (var i = 0; i < children.length; ++i) {
      children[i].cSymbolTable;
    }
    return;
  },
  _terminal: function() {
...
var cTranslator = {
  ...
  Prototype: function(specs, pointer, id, _1, params, _2, _3) {
    return specs.cTranslator + ' ' + pointer.cTranslator + '  ' + id.cTranslator + '(' + params.cTranslator + ');'
  },
  // Abs_declarator_PointerListDirAbsDeclarator: function(pointer, decl, gcc1, gcc2) {
  //   console.log(decl.cTranslator);
  ...
  Struct_decl: function(preprocess, spec, decls, _) {
    return spec.cTranslator + ' ' + decls.cTranslator;
  },
  Spec_qual_list: function(qualifiers, type) {
    return parseArrayList(qualifiers, ' ') + type.cTranslator;
  },
  ...
  Assign_stmt: function(left, _1, right, _4) {
  ...
  Gcc_asm_expr: function(_1, type_qual, _2, strings, _3) {
    // return '';
    var code = '';
    for (var i = 0; i < strings.cTranslator.length; i++) {
      if(strings.cTranslator[i] != '"$UNIX2003"') {
        code += ' ' + strings.cTranslator[i];
      }
    }
    return ' __asm' + type_qual.cTranslator + '(' + code + ')';
  },
  ...
  Function_def: function(specs, decl, stmts) {
    ...
    return specs.cTranslator + " " + declaration + " " + stmts.cTranslator;
  },
  _nonterminal: function(children) {
