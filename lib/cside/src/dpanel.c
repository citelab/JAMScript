...
void dpanel_uaddall(dpanel_t* dp) { // add all pending uflow objects to outgoing redis queue
	...
	while (!last && overrun) {
		...
		redisAsyncCommand(dp->uctx, dpanel_ucallback, dp, "fcall uf_write 1 %s %" PRIu64 " %d %d %f %f %b", uobj->key, uobj->clock, dp->logical_id, dp->logical_appid, cn->xcoord, cn->ycoord, (uint8_t*) uobj->value, (size_t) uobj->len);
	    } else {
		// send without a callback for pipelining.
		redisAsyncCommand(dp->uctx, dpanel_uerrorcheck, NULL, "fcall uf_write 1 %s %" PRIu64 " %d %d %f %f %b", uobj->key, uobj->clock, dp->logical_id, dp->logical_appid, cn->xcoord, cn->ycoord, (uint8_t*) uobj->value, (size_t) uobj->len);
	    }
	    freeUObject(uobj);
	    free(next);
	}
...
// Blocks until our data is ready! That makes a lot more sense.
...
void dfread_struct(dftable_entry_t *df, char *fmt, ...) {
    ...
    if (res.buf != NULL) {
	derror = 0;
	darg_entry_t* dargs = __extract_map(res.buf, res.len),* darg,* tmp;
