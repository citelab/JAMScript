...
var result_logger = new JLogger("results", true, JManager);

JManager.jmanager_add_jlogger(result_logger);
result_logger.jlogger_set_new_log_callback(process_response);

function process_response(response){
     var found = false;
     for(var i = 0; i < registered_devices.length; i++){
       if( response.indexOf(registered_devices[i].dev_id) >= 0){
