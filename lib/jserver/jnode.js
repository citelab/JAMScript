...
function remoteSyncExecute(name, params, expr) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        if (machtype == 'DEVICE') {
            ...
            var tmsg = {"cmd": "REXEC-SYN", "opt": "-", "cond": expr, "actname": name, "actid": "-", "actarg": "-", "args": params};
        ...
        {   
            ...
            var tmsg = {"cmd": "REXEC-ASY", "opt": "-", "cond": expr, "actname": name, "actid": "-", "actarg": "-", "args": params};
...
function machSyncExecute(name, params, expr) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        else 
        {
            ...
            var tmsg = {"cmd": "MEXEC-SYN", "opt": "-", "cond": expr, "actname": name, "actid": "-", "actarg": "-", "args": params};
            var runid = crypto.createHash('md5').update(JSON.stringify(tmsg)).digest('hex');
            // set the actid to runid so we can find get the correct runTable entry at acknowledgment
            tmsg.actid = runid
...
function machAsyncExec(name, params, expr) {
    ...
    {
        // Create the execution call.. message 
        var tmsg = {"cmd": "MEXEC-ASY", "opt": "-", "cond": expr, "actname": name, "actid": "-", "actarg": "-", "args": params};
        var runid = crypto.createHash('md5').update(JSON.stringify(tmsg)).digest('hex');
...
function startService() {
    ...
    mserv.on('connect', function() {
        ...
        setInterval(function() {
            var tmsg = {"cmd": "PING", "opt": "BROKER", "cond": "-", "actname": "-", "actid": "-", "actarg": "-", "args": []};
            var encode = cbor.encode(tmsg);
...
function getResults(cmsg, callback) {
	...
	if (res === undefined) 
		nmsg = {"cmd": "REXEC-RES-PUT", "opt": "-", "cond": "-", "actname": cmsg["actname"], 
