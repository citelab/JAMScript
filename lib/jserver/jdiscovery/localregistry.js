...
LocalRegistry.prototype._scan = function(self) {
    ...
    if (Object.keys(self.attrsToDiscover.device).length !== 0) {
        ...
        for (var i = 0; i < constants.localStorage.numBins; i++) {
            ...
            self._makeDiscoveries(self, binName, machs, self.attrsToDiscover.device);
        }
    }
    ...
    if (Object.keys(self.attrsToDiscover.fog).length !== 0) {
        ...
        for (var i = 0; i < constants.localStorage.numBins; i++) {
            binName = baseName + i;
            machs = JSON.parse(self.localStorage.getItem(binName));
            self._makeDiscoveries(self, binName, machs, self.attrsToDiscover.fog);
        }
    }
    ...
    if (Object.keys(self.attrsToDiscover.cloud).length !== 0) {
        ...
        for (var i = 0; i < constants.localStorage.numBins; i++) {
            binName = baseName + i;
            machs = JSON.parse(self.localStorage.getItem(binName));
            self._makeDiscoveries(self, binName, machs, self.attrsToDiscover.cloud);
        }
    }
...
LocalRegistry.prototype._makeDiscoveries = function(self, binName, machs, dattrs) {
    ...
    for (var machId in machs) {
        if ((now - machs[machId].lastCheckIn) > 10 * constants.localStorage.checkInInterval) {
            // remove from local storage (keep local storage trimmed!)
            self._removeOfflineNode(self, binName, machId);
        } else {
...
LocalRegistry.prototype._removeOfflineNode = function(self, binName, machId) {
