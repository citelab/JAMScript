...
Device.prototype.register = function() {
    ...
    var client = mqtt.connect(globals.mqttBrokerUrl, getConnectionOptions(this.app, this.type, this.id));
        ...
        for (var sub in self.subs) {
            ...
            if (sub.regex.test(topic)) {
                ...
                if (sub.isUserDefined) {
                    ...
                    self.emit(sub.emitTag, topic, message);
                } else {
                    // call the associated built-in function
                    sub.exec(topic, message);
                }
                return;
            }
        }
