...
module.exports =  class JAMIopad extends JAMDatasource{
	...
	getDatasource(key, f){
		...
		console.log("function:",funcString);
	...
	deserializeFunc(funcString){
		...
		*/
		return JSON.parse(funcString, function (key, value) {
			if (value && typeof value === "string" && value.substr(0,8) == "function") {
				var startBody = value.indexOf('{') + 1;
				var endBody = value.lastIndexOf('}');
				var startArgs = value.indexOf('(') + 1;
				var endArgs = value.indexOf(')');
				console.log("args:",value.substring(startArgs, endArgs));
				console.log("body:", value.substring(startBody, endBody));
