...
// Start the JAMCloud discovery agent.. 
// We register this node and get information about other nodes from there..
if (cmdopts.registry !== undefined) {
	var creg = jutils.initRegistry(cmdopts.regserver);
	creg.register();
	creg.updateRegistry();
}

if (cmdopts.cloudType !== undefined) {
	jserver.startService();
} else {
	jserver.doRegister();
	jserver.startService();
}
...
// On connection do the subscription again..
mcli.on('connect', function() {
	mcli.subscribe('/admin/request', {qos:1, retain:false});
	mcli.subscribe('/level/functions/request', {qos:1, retain:false});
});

mcli.on('message', function(topic, buf) {

	cbor.decodeFirst(buf, function(error, msg) {

		try {
			adminProcessor(mcli, msg, function(obj) {
				var encode = cbor.encode(obj);
				mcli.publish('/admin/reply/unknown', encode);		// TODO: Is this ok??
			});
		} catch (e) {
			console.log("error: ", e);			
		}
...
function adminProcessor(mclient, msg, callback) {
		...
		case "REGISTER":
			if (msg["opt"] === "DEVICE") {
				...
				startReqService(port, function() {
					...
					if(device.parent_address.length > 0){
...
function createRexecKillMsg(runid) {
	...
	var nmsg = {"cmd": "REXEC-KILL", "opt": "SYN", "actname": "ACTIVITY", 
	"actid": "__", "actarg": runid, "args": []};
	return nmsg;
}
...
function replyExecReady(sock, cmsg, ren) {
	...
	var nmsg;
	if (ren === undefined) {
		nmsg = {"cmd": "REXEC-QUI", "opt": "ASY", "actname": "ACTIVITY", 
		"actid": cmsg["actid"], "actarg": cmsg["actarg"], "args": []};
	}
	else
	{
		if (checkReadyPolicy(ren))
			nmsg = {"cmd": "REXEC-STA", "opt": "ASY", "actname": "ACTIVITY", 
			"actid": cmsg["actid"], "actarg": cmsg["actarg"], "args": []};
		else
			nmsg = {"cmd": "REXEC-QUI", "opt": "ASY", "actname": "ACTIVITY", 
			"actid": cmsg["actid"], "actarg": cmsg["actarg"], "args": []};
...
function replySyncExecReady(sock, cmsg, ren) {
	...
	if (ren === undefined) {
		nmsg = {"cmd": "REXEC-QUI", "opt": "SYN", "actname": "ACTIVITY", 
		"actid": cmsg["actid"], "actarg": cmsg["actarg"], "args": []};
	}
	else
	{
		if (checkReadyPolicy(ren)) {
			...
			nmsg = {"cmd": "REXEC-STA", "opt": "SYN", "actname": "ACTIVITY", 
			"actid": cmsg["actid"], "actarg": cmsg["actarg"], "args": []};
		}
		else
			nmsg = {"cmd": "REXEC-QUI", "opt": "SYN", "actname": "ACTIVITY", 
