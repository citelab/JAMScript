...
JLogger.prototype = {
	...
	jlogger_get_newest_value: function(){
		if(this.set_size == 0)
			throw "Empty Set";
		return this.data_value[this.data_value.length - 1];
	},
	jlogger_get_value_at: function(index){
		if(this.set_size == 0)
			throw "Empty Set";
		if(index < 0 || index > this.data_value.length - 1)
			throw "Invalid Index";
		return this.data_value[index];
	},

	jlogger_get_range_values: function(start, range){
		if(start < 0 || start + range >= this.data_value.length)
			throw "Invalid Range ...";
	...
	jlogger_process_zrange_response: function(e, response, logger){
		...
		if(e){
			...
			for(var i = 0; i < response.length; i++){
				...
				console.log(logger.index_of_last_value);
				/*
