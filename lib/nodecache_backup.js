...
class NodeCache {
  ...
  localRegistryMapChange(reggieState) {
    ...
    if(this.reggie.connectedLocalRegistries.size == 0) {
      if(this.reggie.localRegistriesMap.size <= fogElevationThreshold) {
        ...
        } else if(this.reggie.localRegistriesMap.size == 0) {
          console.log("\nWARNING: There are no Local Registries on the network!\n");
        }
      } else {
        // Just choosing the first node for now.
        for(let entry of this.reggie.localRegistriesMap) {
          if(entry[0] != jsys.id){
