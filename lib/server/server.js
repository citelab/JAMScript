...
var testf = function hello(self, sock) {
	...
	switch (cmd) {
		...
		appdb.register(appname, function(appid) {
			if (appid !== undefined) {
				...
				svlmgr.createservlet(appid, [{name:testf.name, func:testf}], function(servinfo) {
		...
		appdb.openapp(appname, function(sinfo) {
			if (sinfo !== undefined) {
				...
				svlmgr.recreateservlet(sinfo.appid, {server: sinfo.server, port: sinfo.port}, [{name:testf.name, func:testf}], function(servinfo) {
					if (servinfo !== undefined) {
