...
class JAMCore {
    ...
    addWorker(worker) {
        ...
        this.worker.postMessage([{cmd: CmdNames.SET_JSYS, data: {type: this.jamsys.machtype, tags: this.jamsys.tags, long: this.jamsys.long, lat: this.jamsys.lat}}]);
    ...
    processWorkerMsg(r) {
        switch (r.cmd) {
            ...
            case CmdNames.MEXEC_ACK:
                this.processWorkerAck(r.nodeid + r.taskid, r);
                break;
    ...
    makeServer(id, ip, port) {
        ...
        const rport = constants.multicast.busrPort; //16001
        const sport = constants.multicast.bussPort; //16501

        server.serv.listener = udp.createSocket({type:"udp4", reuseAddr:true}),
        server.serv.sender = udp.createSocket({type:"udp4", reuseAddr:true});
        ...
        server.serv.send = function(data) 
        {
    ...
    async startMainProcessors(sock) {
        ...
        //     return false;
        // });
        sock.listener.on('message', function(buf, err) {
            let qmsg = cbor.decode(buf);
            messageProcessor(that, sock, null, qmsg);
        });
...
async function messageProcessor(jcore, sock, topic, msg) {
    ...
    if ((msg.cmd > CmdNames.EXEC_CMDS_BEG) && (msg.cmd < CmdNames.EXEC_CMDS_END)) {
        ...
        console.log("received an execution!");
        rmsg = await jcore.jdaemon.requestProcessor(msg);
        sock.send(cbor.encode(rmsg));
        if (rmsg.cmd === CmdNames.MEXEC_ACK || rmsg.cmd === CmdNames.REXEC_ACK) {
            ...
            sock.send(cbor.encode(fmsg));
            //sock.publish('/' + cmdOpts.app + '/replies/down', cbor.encode(fmsg));
        }
    } else {
        console.log("Shouldn't have non exec messages right now");
    }
    return true;
}

async function __OLD__messageProcessor(jcore, sock, topic, msg) {
    let rmsg = undefined;
    switch (topic) {
