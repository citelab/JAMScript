...
module.exports = new function parseArgs() {
    ...
    const optdefs = [
        ...
        { name: 'localregistryhost', type: Number}, //TODO: make boolean
        { name: 'local_registry', type: Boolean},
        { name: 'global_registry', type: Boolean},
    ...
    try {
        ...
        if (options.app === undefined) {
            log.error("ERROR! App name should be specified");
            process.exit(1);
        }
        // If fog or cloud is not specified, device is the default type

    } catch(e) {
        log.error("Error processing commandline input: " + e.name);
        // there was an error.. what is it?
        process.exit(1);
    }

    //TODO: fix this sketchiness @FIX
    options.localregistryhost = options.localregistryhost==1 ? true : false;

    // Setup some more defaults
    if (!options.fog && !options.cloud && !options.local_registry && !options.global_registry)
        options.device = true;
