...
class JCoreDaemon {
    ...
    async replyProcessor(msg) {
        ...
        switch (msg['cmd']) {
            ...
            case CmdNames.MEXEC_RES:
                this.jcore.otasktbl.processRes(msg.nodeid + msg.taskid, msg.data);
            break;
        }
    }
    ...
    async requestProcessor(msg) {
        ...
        console.log("Id ", id, " . nodeid ", msg.nodeid, " . taskid ", msg.taskid);
        let res = undefined;
        switch (msg['cmd']) {
            case CmdNames.REXEC:
                res = __INQ_lookup(this.jcore.itaskq, id);
                if (res !== undefined) {
                    console.log("-!!!!!!!!!!!!!!!!!!!!!!!!!! ..... lookup found...", id);
                    // there is an instance of the same task - that is either completed or running
                    if (res.state === INQ_States.COMPLETED)
                        return {cmd: CmdNames.REXEC_RES, nodeid:msg.nodeid, taskid: msg.taskid, data: res.results};
                    else 
                        return {cmd: CmdNames.REXEC_ACK, nodeid:msg.nodeid, taskid: msg.taskid, data: Timeouts.REXEC_ACK_TIMEOUT};
                } else {
                    ...
                    if (fentry.sideeffect === false) {
                        ...
                        if (presults !== undefined && Math.random() < presults.prob)
                            return {cmd: CmdNames.REXEC_ACK_RES, nodeid:msg.nodeid, taskid: msg.taskid, fn_argsig: "i", args: [presults.res]};
                    }
    ...
    async new_execution(msg, id) {
        let that = this;
        this.jcore.enqueueJob({cmd: msg.cmd, fn_name: msg.fn_name, nodeid: msg.nodeid, taskid: msg.taskid, params: msg.params});
