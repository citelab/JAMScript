        ...
        switch (msg['cmd']) {
            case CmdNames.REXEC:
                ...
                if (res !== undefined) {
                    ...
                    if (res.state === INQ_States.COMPLETED)
                        yield JAMP.createRemotePutRes(msg.taskid, this.jcore.jamsys.id, msg.nodeid, res.results);
                    else
                    ...
                    if (pres && Math.random() < pres.prob)
                        yield JAMP.createRemotePutRes(msg.taskid, this.jcore.jamsys.id, msg.nodeid, pres.results)
                    ...
                    if (res !== undefined && res.cmd === CmdNames.REXEC_ACK) {
                        ...
                        if (msg.subcmd === 1) {
                            ...
                            yield JAMP.createRemotePutRes(msg.taskid, this.jcore.jamsys.id, msg.nodeid, res);
                        } 
                    } else 
            ...
            case CmdNames.GET_REXEC_RES:
                ...
                if (res !== undefined) {
                    // there is an instance of the same task - that is either completed or running
                    if (res.state === INQ_States.COMPLETED)
                        yield JAMP.createRemotePutRes(msg.nodeid, msg.taskid, this.jcore.jamsys.id, res.results);
                    else {
                        let cres = await this.checkExecution(msg);
                        this.jcore.intasktbl.updatePCache(msg.fn_name, msg.params, true, cres);
                        yield JAMP.createRemotePutRes(msg.nodeid, msg.taskid, this.jcore.jamsys.id, cres);
                    }
                } else 
            ...
            case CmdNames.MEXEC:
                ...
                if (res !== undefined) {
                    // there is an instance of the same task - that is either completed or running
                    if (res.state === INQ_States.COMPLETED)
                        yield JAMP.createMachPutRes(msg.taskid, this.jcore.jamsys.id, msg.nodeid, res.results);
                    else
                        yield JAMP.createMachPutAck(msg.taskid, this.jcore.jamsys.id, msg.nodeid, Timeouts.REXEC_ACK_TIMEOUT);
                } else {
                    let pres = this.jcore.intasktbl.findApprox(msg.taskid, msg.fn_name, msg.params);
                    if (pres && Math.random() < pres.prob)
                        yield JAMP.createMachPutRes(msg.taskid, this.jcore.jamsys.id, msg.nodeid, pres.results)
                    ...
                    if (res !== undefined && res.cmd === CmdNames.MEXEC_ACK) {
                        ...
                        if (msg.subcmd === 1) {
                            let res = await this.checkExecResults(msg.taskid);
                            yield JAMP.createMachPutRes(msg.taskid, this.jcore.jamsys.id, msg.nodeid, res);
                        }
                    } else 
            ...
            case CmdNames.GET_MEXEC_RES:
                ...
                if (res !== undefined) {
                    ...
                    if (res.state === INQ_States.COMPLETED)
                        ...
                        let cres = await this.checkExecution(msg);
                        this.jcore.intasktbl.updatePCache(msg.fn_name, msg.params, true, cres);
                        yield JAMP.createMachPutRes(msg.taskid, this.jcore.jamsys.id, msg.nodeid, cres);
                    }
                } else 
