...
class JCoreDaemon {
        ...
        switch (msg['cmd']) {
            case CmdNames.REXEC:
                ...
                if (res !== undefined) {
                    ...
                    if (res.state === INQ_States.COMPLETED)
                        ...
                        return JAMP.createRemotePutAck(msg.nodeid, msg.taskid, this.jcore.jamsys.id, Timeouts.REXEC_ACK_TIMEOUT);
                } else {
                    let pres = this.jcore.intasktbl.findApprox(msg.taskid, msg.fn_name, msg.params);
                    if (pres && Math.random() < pres.prob)
                        return JAMP.createRemotePutRes(msg.nodeid, msg.taskid, this.jcore.jamsys.id, pres.results)
                    ...
                    if (res !== undefined && res.cmd === CmdNames.REXEC_ACK)
                        ...
                        return JAMP.createRemotePutAck(msg.taskid, this.jcore.jamsys.id, msg.nodeid);
                    else 
            ...
            case CmdNames.GET_REXEC_RES:
                ...
                if (res !== undefined) {
                    ...
                    if (res.state === INQ_States.COMPLETED)
                        ...
                        this.jcore.intasktbl.putCache(msg.fn_name, msg.params, true, cres);
            ...
            case CmdNames.MEXEC:
                ...
                if (res !== undefined) {
                    // there is an instance of the same task - that is either completed or running
                    if (res.state === INQ_States.COMPLETED)
                        return JAMP.createMachPutRes(msg.nodeid, msg.taskid, this.jcore.jamsys.id, res.results);
                    else
                        return JAMP.createMachPutAck(msg.nodeid, msg.taskid, this.jcore.jamsys.id, Timeouts.REXEC_ACK_TIMEOUT);
                } else {
                    let pres = this.jcore.intasktbl.findApprox(msg.taskid, msg.fn_name, msg.params);
                    if (pres && Math.random() < pres.prob)
                        return JAMP.createMachPutRes(msg.nodeid, msg.taskid, this.jcore.jamsys.id, pres.results)
                    ...
                    if (res !== undefined && res.cmd === CmdNames.MEXEC_ACK)
                        // we are returning after ACK.. not completion.. the worker thread is sending the ACK at start of task
                        return JAMP.createMachPutAck(msg.taskid, this.jcore.jamsys.id, msg.nodeid);
                    else 
            ...
            case CmdNames.GET_MEXEC_RES:
                ...
                if (res !== undefined) {
                    ...
                    if (res.state === INQ_States.COMPLETED)
                        ...
                        let cres = await this.checkExecution(msg);
                        this.jcore.intasktbl.putCache(msg.fn_name, msg.params, true, cres);
                        return JAMP.createMachPutRes(msg.nodeid, msg.taskid, this.jcore.jamsys.id, cres);
                    }
                } else 
                    return JAMP.createMachPutErr(msg.nodeid, msg.taskid, this.jcore.jamsys.id, ErrorCodes.NoTask);
            break;
        }
    ...
    async newExecution(msg) {
        ...
        return new Promise((resolve, reject)=> {
            ...
            that.jcore.intasktbl.put(msg.taskid, INQ_States.STARTED, undefined, function(state, res) {
