        ...
        switch (msg['cmd']) {
            case CmdNames.REXEC:
                ...
                if (res !== undefined) {
                    ...
                    if (res.state === INQ_States.COMPLETED)
                        return JAMP.createRemotePutRes(msg.taskid, this.jcore.jamsys.id, msg.nodeid, res.res_sig, res.results);
                    else
                    ...
                    if (pres && Math.random() < pres.prob)
                        return JAMP.createRemotePutRes(msg.taskid, this.jcore.jamsys.id, msg.nodeid, pres.res_sig, pres.results)
            ...
            case CmdNames.GET_REXEC_RES:
                ...
                if (res !== undefined) {
                    // there is an instance of the same task - that is either completed or running
                    if (res.state === INQ_States.COMPLETED)
                        return JAMP.createRemotePutRes(msg.nodeid, msg.taskid, this.jcore.jamsys.id, res.res_sig, res.results);
                    else {
                        let cres = await this.checkExecResults(msg);
                        this.jcore.intasktbl.updatePCache(msg.fn_name, msg.params, true, cres);
                        return JAMP.createRemotePutRes(msg.nodeid, msg.taskid, this.jcore.jamsys.id, cres.res_sig, cres.results);
                    }
                } else
            ...
            case CmdNames.MEXEC:
                ...
                if (res !== undefined) {
                    // there is an instance of the same task - that is either completed or running
                    if (res.state === INQ_States.COMPLETED)
                        return JAMP.createMachPutRes(msg.taskid, this.jcore.jamsys.id, msg.nodeid, res.res_sig, res.results);
                    else
                        return JAMP.createMachPutAck(msg.taskid, this.jcore.jamsys.id, msg.nodeid, Timeouts.REXEC_ACK_TIMEOUT);
                } else {
                    let pres = this.jcore.intasktbl.findApprox(msg.taskid, msg.fn_name, msg.params);
                    if (pres && Math.random() < pres.prob)
                        return JAMP.createMachPutRes(msg.taskid, this.jcore.jamsys.id, msg.nodeid, pres.res_sig, pres.results)
            ...
            case CmdNames.GET_MEXEC_RES:
                ...
                if (res !== undefined) {
                    ...
                    if (res.state === INQ_States.COMPLETED)
                        ...
                        let cres = await this.checkExecResults(msg);
                        this.jcore.intasktbl.updatePCache(msg.fn_name, msg.params, true, cres);
                        return JAMP.createMachPutRes(msg.taskid, this.jcore.jamsys.id, msg.nodeid, cres.res_sig, cres.results);
                    }
                } else
