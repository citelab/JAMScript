        ...
        switch (msg['cmd']) {
            case CmdNames.REXEC:
                ...
                if (res !== undefined) {
                    ...
                    if (res.state === INQ_States.COMPLETED)
                        return JAMP.createRemotePutRes(msg.taskid, this.jcore.jamsys.id, msg.nodeid, res.results);
                    else
                        return JAMP.createRemotePutAck(msg.taskid, this.jcore.jamsys.id, msg.nodeid, Timeouts.REXEC_ACK_TIMEOUT);
                } else {
                    let pres = this.jcore.intasktbl.findApprox(msg.taskid, msg.fn_name, msg.params);
                    if (pres && Math.random() < pres.prob)
                        return JAMP.createRemotePutRes(msg.taskid, this.jcore.jamsys.id, msg.nodeid, pres.results)
                    ...
                    if (res !== undefined && res.cmd === CmdNames.REXEC_ACK) {
                        ...
                        yield JAMP.createRemotePutAck(msg.taskid, this.jcore.jamsys.id, msg.nodeid);
                        if (msg.subcmd === 1) {
                            let res = await this.checkExecResults(msg.taskid);
                            return JAMP.createRemotePutRes(msg.taskid, this.jcore.jamsys.id, msg.nodeid, res);
                        } else 
                            return {cmd: CmdNames.DONE};
                    } else 
                        return JAMP.createRemotePutNak(msg.taskid, this.jcore.jamsys.id, msg.nodeid, res.subcmd);
                }
            break;
            case CmdNames.GET_REXEC_RES:
                ...
                if (res !== undefined) {
                    // there is an instance of the same task - that is either completed or running
                    if (res.state === INQ_States.COMPLETED)
                        return JAMP.createRemotePutRes(msg.nodeid, msg.taskid, this.jcore.jamsys.id, res.results);
                    else {
                        let cres = await this.checkExecution(msg);
                        this.jcore.intasktbl.updatePCache(msg.fn_name, msg.params, true, cres);
                        return JAMP.createRemotePutRes(msg.nodeid, msg.taskid, this.jcore.jamsys.id, cres);
                    }
                } else 
                    return JAMP.createRemotePutErr(msg.nodeid, msg.taskid, this.jcore.jamsys.id, ErrorCodes.NoTask);
            break;
            case CmdNames.MEXEC:
                res = this.jcore.intasktbl.findExact(msg.taskid);
                if (res !== undefined) {
                    // there is an instance of the same task - that is either completed or running
                    if (res.state === INQ_States.COMPLETED)
                        return JAMP.createMachPutRes(msg.taskid, this.jcore.jamsys.id, msg.nodeid, res.results);
                    else
                        return JAMP.createMachPutAck(msg.taskid, this.jcore.jamsys.id, msg.nodeid, Timeouts.REXEC_ACK_TIMEOUT);
                } else {
                    let pres = this.jcore.intasktbl.findApprox(msg.taskid, msg.fn_name, msg.params);
                    if (pres && Math.random() < pres.prob)
                        return JAMP.createMachPutRes(msg.taskid, this.jcore.jamsys.id, msg.nodeid, pres.results)
                    ...
                    if (res !== undefined && res.cmd === CmdNames.MEXEC_ACK) {
                        ...
                        if (msg.subcmd === 1) {
                            let res = await this.checkExecResults(msg.taskid);
                            return JAMP.createMachPutRes(msg.taskid, this.jcore.jamsys.id, msg.nodeid, res);
                        } else 
                            return {cmd: CmdNames.DONE};
                    } else 
                        return JAMP.createMachPutNak(msg.taskid, this.jcore.jamsys.id, msg.nodeid, res.subcmd);
                }
            break;
            case CmdNames.GET_MEXEC_RES:
                ...
                if (res !== undefined) {
                    ...
                    if (res.state === INQ_States.COMPLETED)
                        ...
                        let cres = await this.checkExecution(msg);
                        this.jcore.intasktbl.updatePCache(msg.fn_name, msg.params, true, cres);
                        return JAMP.createMachPutRes(msg.taskid, this.jcore.jamsys.id, msg.nodeid, cres);
                    }
                } else 
                    return JAMP.createMachPutErr(msg.taskid, this.jcore.jamsys.id, msg.nodeid, ErrorCodes.NoTask);
            break;
        }
        return JAMP.createUnknownErr();
    }
    ...
    async checkExecResults(id) {
        ...
        if (res.state === INQ_States.COMPLETED)
            ...
            return new Promise((resolve, reject)=> {
                that.jcore.intasktbl.updateCB(id, function(state, res) {
