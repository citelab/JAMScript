...
class JCoreClient {
    ...
    remoteTaskExec(name, argsig, params, nodeid, taskid, rrequired) {
        ...
        this.jcore.otasktbl.insert(nodeid + taskid, rrequired, function cb(code, val) {
            switch (code) {
                case StateNames.TIMEOUT:
                    that.jcore.enqueueJob({cmd: CmdNames.TIMEOUT, nodeid: nodeid, taskid: taskid});
                break;
