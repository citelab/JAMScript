...
class JCoreClient {
        ...
        this.jcore.otasktbl.insert(nodeid + taskid, rrequired, function cb(code, nid, val) {
            switch (code) {
                case StateNames.TIMEOUT:
                    that.jcore.enqueueJob({cmd: CmdNames.REXEC_ERR, nodeid: nid, taskid: taskid});
                break;
    ...
    //    console.timeLog(taskid);
        this.jcore.otasktbl.insert(nodeid + taskid, rrequired, function cb(code, nid, val) {
            switch (code) {
                case StateNames.TIMEOUT:
                    that.jcore.enqueueJob({cmd: CmdNames.MEXEC_ERR, nodeid: nid, taskid: taskid});
                break;
                case StateNames.ERR_RECVD:
                    that.jcore.enqueueJob({cmd: CmdNames.MEXEC_ERR, nodeid: nid, taskid: taskid});
                break;
    ...
    __sendMachRequest(tmsg, jcore) {
        console.log("Sending Mach Request... ", tmsg);
        ...
        if (jcore.jamsys.machtype === globals.NodeType.DEVICE) {
            ...
            console.log("Number of fogs ...", jcore.fogs.size);
