    ...
    doRegister() {
        ...
        if (this.jcore.jamsys.machtype === globals.NodeType.DEVICE) {
            this.jcore.jamsys.reggie.on('fog-up', function(id, info) {
                console.log("===========> Fog Up.. message first arrived ..", id, info);
                that.jcore.ncache.fogUp(id, info);
            });
            this.jcore.jamsys.reggie.on('fog-down', function(id) {
                console.log("===========> Fog Down.. message first arrived ..", id);
        ...
        if (this.jcore.jamsys.machtype === globals.NodeType.DEVICE) {
            ...
            if (drecord !== undefined && drecord.tag === "none") {
                ...
                ebus.on('fog_up', function(fogId, connInfo) {
                    ...
                    console.log("Fog up message...", rmsg);
                    callback(rmsg);
                });
                ebus.on('fog_down', function(id, connInfo) {
                    console.log("===================================== fog down... ", id, connInfo);
                    var rmsg = JAMP.createFogDeletionPCFI(id, connInfo.ip, connInfo.port);
                    console.log("Fog delete message... ", rmsg);
                    callback(rmsg);
                });
                ...
                ebus.on('cloud_down', function(id, connInfo) {
                    var rmsg = JAMP.createCloudDeletionPCFI(id, connInfo.ip, connInfo.port);
                    console.log("Cloud up message.. ", rmsg);
                    callback(rmsg);
                });
