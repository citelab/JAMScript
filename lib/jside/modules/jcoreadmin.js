...
class JCoreAdmin {
    ...
    async getAdminProcessor(msg) {
        let rdevid = msg['nodeid'];
        if (rdevid !== undefined) {
            let entry = this.devTable.has(rdevid);
            if (entry)
                return entry.ahandle;
            else {
                if (msg.cmd === CmdNames.REGISTER) {
                    let ahandle = this.adminProcessor(msg);
                    this.devTable.set(rdevid, {time: Date.now(), tag: "none", ahandle: ahandle});
                    return ahandle;
                } else 
                    return undefined;
            }
