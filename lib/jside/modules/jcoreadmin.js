...
class JCoreAdmin {
    ...
    async *adminProcessor() {
        while (1)
            yield await this.__getCloudFogInfo();
    }

    // there is only one adminProcessor.. 
    async getAdminProcessor(msg) {
        let rdevid = msg['nodeid'];
        if (rdevid !== undefined) {
            let ahandle = this.adminProcessor();
    ...
    async processPong(msg) {
        let entry = this.devTable.get(msg.nodeid);
        if (entry !== undefined)
            entry.used = true;
    }
}
