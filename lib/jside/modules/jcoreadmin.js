...
class JCoreAdmin {
    ...
    doRegister() {
        ...
        this.jcore.ncache.onFogDataUp(function(id, info) {
            that.jcore.enqueueJob({cmd: CmdNames.REDIS_STATE, opt: CmdNames.FOG_DATA_UP, data: info});
            ebus.fogDataUp(id, info);
        });

        this.jcore.ncache.onFogDataDown(function(id) {
            that.jcore.enqueueJob({cmd: CmdNames.REDIS_STATE, opt: CmdNames.FOG_DATA_DOWN});
            ebus.fogDataDown(id);
        });
