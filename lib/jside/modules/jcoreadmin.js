    ...
    doRegister() {
        ...
        if (this.jcore.jamsys.machtype === globals.NodeType.DEVICE) {
            this.jcore.jamsys.reggie.on('fog-up', function(id, info) {
                console.log("Fog up receviev...", id, info);
            ...
            this.jcore.jamsys.reggie.on('new-loc', function(id, info) {
                that.jcore.ncache.fogChanged(id, info);
            });
        ...
        if (this.jcore.jamsys.machtype === globals.NodeType.DEVICE) {
            ...
            if (drecord !== undefined && drecord.tag === "none") {
                ...
                ebus.on('fog_up', function(fogId, connInfo) {
                    console.log("Fog... up sent....");
                    var rmsg = JAMP.createFogAdditionPCFI(that.fogInfoCount, [helper.getURL(connInfo.ip, connInfo.port)]);
                    callback(rmsg);
                });
                ebus.on('fog_down', function(id, connInfo) {
                    var rmsg = JAMP.createFogDeletionPCFI(that.fogInfoCount, [helper.getURL(connInfo.ip, connInfo.port)]);
                    callback(rmsg);
                });
                ebus.on('cloud_up', function(cloudId, connInfo) {
                    console.log("Sending cloud Addtion.. PCFI", that.cloudInfoCount);
                    var rmsg = JAMP.createCloudAdditionPCFI(that.cloudInfoCount, [helper.getURL(connInfo.ip, connInfo.port)]);
                    callback(rmsg);
                });
                ebus.on('cloud_down', function(id, connInfo) {
                    var rmsg = JAMP.createCloudDeletionPCFI(that.cloudInfoCount, [helper.getURL(connInfo.ip, connInfo.port)]);
                    callback(rmsg);
                });
                ebus.on('none_up', function() {
                    var rmsg = JAMP.createNonePCFI();
                    callback(rmsg);
                })
            }
            ebus.trigger();
