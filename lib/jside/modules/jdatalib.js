...
async function __configureListener(r, l, myid) {
    return new Promise((resolve, reject) => {
        ...
        l.call('psubscribe', '__keyevent*', function(err, resp) {
            if (resp[3] !== undefined) {
...
class UFlow {
    ...
    async write(value) {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.rs.call('fcall', 'uf_write', 1, this.key, clock, this.id, scope, jsys.long, jsys.lat, val, function(err, resp) {
                if (err === null)
                    resolve(resp);
                else 
                    resolve(null);
...
class DFlow {
    ...
    async write(value) {
        ...
        return new Promise((resolve, reject) => {
            let cval = cbor.encode(value);
            let val = cval.toString('base64');
            this.rs.call('fcall', 'df_write', 1, this.key, clock, this.id, scope, jsys.long, jsys.lat, val, function(err, resp) {
                if (err === null)
                    resolve(resp);
                else 
                    resolve(null);
