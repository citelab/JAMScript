...
async function updateUServers(x, uid, app) {
    ...
    switch (x.cmd) {
        case CmdNames.FOG_DATA_UP:
            ...
            aux.on('error', (e)=> {
                console.log("Error updating the fog servers...", err);
            });
        break;
        ...
        case CmdNames.CLOUD_DATA_UP:
            ...
            aux.on('error', (err)=> {
                console.log("Error updating the cloud servers.. ", err);
            });
        break;
...
async function configureMainRedis(main, host, port, appid, cb) {
    ...
    main.on("error", (e)=> { console.log("Error configuring main server... ", e); });
...
async function configureAuxRedis(aux, host, port, lmyid, appid) {
    let listen = new Redis(host, port, {retryStrategy: ()=> { return false; }, reconnectOnError: ()=> { return false; }});
    listen.on("error", ()=> { console.log("error..."); });
                ...
                if (entry !== undefined) {
                    switch (entry.type) {
                        case globals.RedisFuncs.UF_READ_FIRST:
                            r.call('fcall', 'uf_fread', 1, key, appid, function(err, res) {
                                if (err === null)
                                    entry.callback(res);
                            });
                            break;
                        case globals.RedisFuncs.UF_READ_LAST:
                            r.call('fcall', 'uf_lread', 1, key, appid, function(err, res) {
                                if (err === null)
                                    entry.callback(res);
                            });
                            break;
                ...
                if (entry !== undefined) {
                    switch (entry.type) {
                        case globals.RedisFuncs.DF_READ_TAIL:
                            r.call('fcall', 'df_lread', 1, key, aid, function(err, res) {
                                if (err === null)
                                    entry.callback(res);
                            });
                            break;
                        case globals.RedisFuncs.DF_READ_HEAD:
                            r.call('fcall', 'df_fread', 1, key, myid, aid, function(err, res) {
                                if (err === null)
                                    entry.callback(res);
                            });
                            break;
...
class UFlow {
    ...
    __write(rserv, clock, value) {
        return new Promise((resolve, reject) => {
            ...
            rserv.call('fcall', 'uf_write', 1, this.key, clock, this.id, this.appid, 1, jsys.long, jsys.lat, val, function(err, resp) {
                if (err === null)
                    resolve(resp);
                else
                    resolve(null);
    ...
    async read() {
        ...
        return new Promise((resolve, reject) => {
            this.rs.call('fcall', 'uf_randread', 1, this.key, this.appid, clock, function(err, resp) {
                if (err === null) {
...
class DFlow {
    ...
    async write(value) {
        ...
        return new Promise((resolve, reject) => {
            let cval = cbor.encode(value);
            let val = cval.toString('base64');
            this.rs.call('fcall', 'df_write', 1, this.key, clock, this.id, this.appid, jsys.long, jsys.lat, val, function(err, resp) {
                if (err === null)
                    resolve(resp);
                else
                    resolve(null);
