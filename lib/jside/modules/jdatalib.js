    ...
    return new Promise((resolve, reject) => {
        ...
        l.on('message', (channel, key) => {
            if (channel === appid + "__keycompleted") {
                let [k, cnt] = key.split('_###_');
                console.log("Key ", k, " Count ", cnt);
                let entry = fltable.get(k);
                console.log("Got notification... ", k, cnt, entry);
                if (entry !== undefined) {
                    switch (entry.type) {
                        case globals.RedisFuncs.UF_READ_FIRST:
                            if (entry.count === undefined || entry.count <= cnt)
                                r.callBuffer('fcall', 'uf_fread', 1, k, appid, function(err, res) {
                                    if (err === null && res !== [])
                                        entry.callback(res);
                                });
                            break;
                        case globals.RedisFuncs.UF_READ_LAST:
                            if (entry.count === undefined || entry.count <= cnt)
                                r.callBuffer('fcall', 'uf_lread', 1, k, appid, function(err, res) {
                                    if (err === null && res !== [])
                                        entry.callback(res);
                                });
                            break;
...
async function getWCount(rserv) {
    return rserv.call('fcall', 'getwcount', 0);
}
