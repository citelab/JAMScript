        ...
        return new Promise((resolve, reject) => {
            let cb = function(resp) {
                ...
                for(let i = 0; i < resp.length; i++) {
                    if (node_buffer.isAscii(resp[i]) === true && resp[i].toString().startsWith("value###") === true) {
                        // console.log("reading buf: ", resp[i+1].toString("hex"));
                        y.push(cbor.decode(resp[i+1]));
                    }
                }
                resolve(y);
            }
...
class DFlow {
    ...
    async write(value) {
        ...
        return new Promise((resolve, reject) => {
            let cval = cbor.encode(value);
            // console.log("writing val", cval.toString("hex"));
