...
async function* machExecRV(name, ...params) {
    ...
    try {
        ...
        wchan.on('gotErr', (tid, ecode)=> {
            status.pending--;
            if (status.pending <= 0)
                return Promise.reject(new Error(ecode));
            else 
                yield Promise.reject(new Error(ecode));
        });
        wchan.on('gotTimeout', (tid)=> {
            status.pending--;
            if (status.pending <= 0)
...
async function* remoteExecRV(name, argsig, ...params) {
    ...
    let taskid = Date.now() * 1000 + taskIdCounter++;
    try {
        let msg = {cmd: CmdNames.REXEC, fn_name: name, fn_argsig: argsig, params: params, taskid: taskid, results: true}
        let wentry = worktbl.createEntry(taskid, msg);
        let wchan = wentry.channel;
        status.pending = wentry.pending;
        wchan.on('gotRes', (tid, y)=> {
