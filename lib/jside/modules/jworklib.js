            ...
            ev.forEach((v)=> {
                switch (v.cmd) {
                    ...
                    case CmdNames.REXEC_RES:
                        if (enabled) {
                            processRexecRes(v.taskid, v.data);
                            parent.postMessage({cmd: CmdNames.DONE});
                        }
                    break;
                    case CmdNames.MEXEC_RES:
                        if (enabled) {
                            processMexecRes(v.taskid, v.data);
                            parent.postMessage({cmd: CmdNames.DONE});
                        }
                    break;
...
function processRexecAck(taskid) {
    let wentry = worktbl.get(taskid);
    worktbl.incrementRexecAckCnt(taskid);
    if (wentry !== undefined) {
        wentry.channel.emit("gotAck");
        if (wentry.record.results)
            worktbl.rewriteTimeout(taskid);
    }
}

function processMexecAck(taskid) {
    let wentry = worktbl.get(taskid);
    worktbl.incrementMexecAckCnt(taskid);
    if (wentry !== undefined) {
        wentry.channel.emit("gotAck");
        if (wentry.record.results)
            worktbl.rewriteTimeout(taskid);
...
function processRexecRes(taskid, val) {
    let wentry = worktbl.get(taskid);
    worktbl.incrementRexecResCnt(taskid);
    if (wentry !== undefined && wentry.record.results === true) 
        wentry.channel.emit("gotRes", val);
    worktbl.rewriteTimeout(taskid);
}

function processMexecRes(taskid, val) {
    let wentry = worktbl.get(taskid);
    worktbl.incrementMexecResCnt(taskid);
    if (wentry !== undefined && wentry.record.results === true) 
        wentry.channel.emit("gotRes", val);
    worktbl.rewriteTimeout(taskid);
}
