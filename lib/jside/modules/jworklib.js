...
async function machExecRV(name, ...params) {
    ...
    return new Promise((resolve, reject)=> {
        ...
        wchan.on('gotTimeout2', (tid)=> {
            console.log("GotTimeout2...");
            worktbl.delete(tid);
            parent.postMessage({cmd: CmdNames.PURGE, taskid: tid, nodeid: jsys.id});
        });
        wchan.on('gotTimeout', (tid)=> {
            console.log("GotTimeout....");
            worktbl.delete(tid);
            parent.postMessage({cmd: CmdNames.PURGE, taskid: tid, nodeid: jsys.id});
            resolve({ status: function() { return {completed: status.completed, error: status.error}; }, values: function() { return values;}});
        });
...
//    console.log("Launching... ", taskid);
    return new Promise((resolve, reject)=> {
        ...
        wchan.on('gotRes', (tid, y)=> {
            ...
            values.push(y);
            if (status.pending <= status.completed) {
//                console.log("                                                       Status for ", tid, "   ", status);
                worktbl.gotoSubWaiting(tid);
                resolve({ status: function() { return {completed: status.completed, error: status.error}; }, values: function() { return values;}});
            }
        });
...
//            console.log("Got error......");
        ...
        wchan.on('gotTimeout', (tid)=> {
//            console.log("                                                       gotTimeout for ", tid, "   ", status)
            worktbl.delete(tid);
            parent.postMessage({cmd: CmdNames.PURGE, taskid: tid, nodeid: jsys.id});
            resolve({ status: function() { return {completed: status.completed, error: status.error}; }, values: function() { return values;}});
        });
...
function executeRV(v, resMsg, errMsg, ackMsg, nackMsg) {
    ...
    if (fentry !== undefined) {
        if ((checkCondition(fentry.cond) !== true))
            ...
            res.then((r)=> {
                console.log("Result ...", {cmd: resMsg, data: r, taskid: v.taskid, nodeid: v.nodeid});
...
function processError(taskid) {
    ...
    if (wentry !== undefined)
        wentry.channel.emit("gotErr", taskid);
}
...
//    console.log("                                                                                                                 ----> process Res --- ", taskid, nodeid, val);
    let wentry = worktbl.get(taskid);
    let res = worktbl.processRes(taskid, nodeid);
    if (res && wentry !== undefined && wentry.record.results === true)
