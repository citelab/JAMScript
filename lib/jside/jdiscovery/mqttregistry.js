...
MQTTRegistry.prototype.cleanup = async function(){
  ...
  if(deletionLeft != 0) {
    console.log("ERROR: Wasn't able to clear all previous retained messages (DBG: "+deletionLeft+")");
  }
...
MQTTRegistry.prototype._prepareForEvents = function (cleanupOnConnect) {
  ...
  this.client.on("close", function() {
    if(this.connected) {
      console.log(`WARNING: mqtt client (${self.brokerUrl}) connection closed!`);
