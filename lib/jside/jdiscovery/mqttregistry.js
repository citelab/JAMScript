...
MQTTRegistry.prototype.registerAndDiscover = function (
  ...
  this.client = mqtt.connect(mqttBrokerUrl, this._getConnectionOptions());
...
MQTTRegistry.prototype._prepareForEvents = function () {
  ...
  this.client.on("connect", function (connack) {
    ...
    if (!connack.sessionPresent) {
      ...
      for (var attr in self.subscribedAttrs.local_registry) {
        self.attrsToSubTo.local_registry[attr] =
          self.subscribedAttrs.local_registry[attr];
      }
      for (var attr in self.subscribedAttrs.global_registry) {
        self.attrsToSubTo.global_registry[attr] =
          self.subscribedAttrs.global_registry[attr];
      }
  ...
  this.client.on("message", function (topic, message, packet) {
    ...
    console.log("Testing: " + message.toString());
...
MQTTRegistry.prototype._subscribeWithRetries = function (
  ...
  for (var attr in dattrs.local_registry) {
    subs[self.app + "/local_registry/+/" + attr] = { qos: self.subQos };
  }

  for (var attr in dattrs.global_registry) {
    subs[self.app + "/global_registry/+/" + attr] = { qos: self.subQos };
  }
  ...
  for (var attr in dattrs.local_registry) {
    delete self.attrsToSubTo.local_registry[attr];
    self.subscribedAttrs.local_registry[attr] = dattrs.local_registry[attr];
  }

  for (var attr in dattrs.global_registry) {
    delete self.attrsToSubTo.global_registry[attr];
    self.subscribedAttrs.global_registry[attr] = dattrs.global_registry[attr];
  }
  ...
  self.client.subscribe(subs, function (err, granted) {
    if (err) {
      if (retries !== 0) {
        ...
        for (var attr in dattrs.local_registry) {
          delete self.subscribedAttrs.local_registry[attr];
          self.attrsToSubTo.local_registry[attr] = dattrs.local_registry[attr];
        }

        for (var attr in dattrs.global_registry) {
          delete self.subscribedAttrs.global_registry[attr];
          self.attrsToSubTo.global_registry[attr] =
            dattrs.global_registry[attr];
        }
      ...
      for (var attr in dattrs.global_registry) {
        delete self.subscribedAttrs.global_registry[attr];
        self.attrsToSubTo.global_registry[attr] = dattrs.global_registry[attr];
      }
...
MQTTRegistry.prototype._unsubscribeWithRetries = function (
  ...
  for (var attr in dattrs.local_registry) {
    topics.push(self.app + "/local_registry/+/" + dattrs.local_registry[attr]);
  }

  for (var attr in dattrs.global_registry) {
    topics.push(
      self.app + "/global_registry/+/" + dattrs.global_registry[attr]
    );
  }
  ...
  self.client.unsubscribe(topics, function (err) {
    if (err) {
      ...
      for (var attr in dattrs.local_registry) {
        delete self.subscribedAttrs.local_registry[attr];
        delete self.attrsToUnsubFrom.local_registry[attr];
      }
      for (var attr in dattrs.global_registry) {
        delete self.subscribedAttrs.global_registry[attr];
        delete self.attrsToUnsubFrom.global_registry[attr];
      }
    }
  });
};
...
MQTTRegistry.prototype.discoverAttributes = function (dattrs) {
  ...
  for (var attr in dattrs.local_registry) {
    // in case this attr is queued up to be unsubscribed from
    delete this.attrsToUnsubFrom.local_registry[attr];
    if (!this.subscribedAttrs.local_registry.hasOwnProperty(attr)) {
      // try to subscribe to it
      if (subs === null) {
        subs = {
          device: {},
      ...
      subs.local_registry[attr] = dattrs.local_registry[attr];
      this.attrsToSubTo.local_registry[attr] = dattrs.local_registry[attr];
    }
  }

  for (var attr in dattrs.global_registry) {
    // in case this attr is queued up to be unsubscribed from
    delete this.attrsToUnsubFrom.global_registry[attr];
    if (!this.subscribedAttrs.global_registry.hasOwnProperty(attr)) {
      // try to subscribe to it
      if (subs === null) {
        subs = {
          device: {},
      ...
      subs.global_registry[attr] = dattrs.global_registry[attr];
      this.attrsToSubTo.global_registry[attr] = dattrs.global_registry[attr];
    }
  }
...
MQTTRegistry.prototype.stopDiscoveringAttributes = function (dattrs) {
  ...
  if (dattrs.local_registry) {
    for (var i = 0; i < dattrs.local_registry.length; i++) {
      delete this.attrsToSubTo.local_registry[dattrs.local_registry[i]];
      if (this.subscribedAttrs.hasOwnProperty(dattrs.local_registry[i])) {
        this.attrsToUnsubFrom.local_registry[dattrs.local_registry[i]] = null;
        if (unsubs === null) {
          unsubs = {
            device: {},
            fog: {},
        ...
        unsubs.local_registry[dattrs.local_registry[i]] = null;
      }
    }
  }

  if (dattrs.global_registry) {
    for (var i = 0; i < dattrs.global_registry.length; i++) {
      delete this.attrsToSubTo.global_registry[dattrs.global_registry[i]];
      if (this.subscribedAttrs.hasOwnProperty(dattrs.global_registry[i])) {
        this.attrsToUnsubFrom.global_registry[dattrs.global_registry[i]] = null;
        if (unsubs === null) {
          unsubs = {
            device: {},
            fog: {},
        ...
        unsubs.global_registry[dattrs.global_registry[i]] = null;
      }
    }
  }
