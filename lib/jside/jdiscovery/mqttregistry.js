...
MQTTRegistry.prototype.restartConnection = function(
  ...
  console.log("Attempting to Intentionally Reconnect to Regstry");

  self.customExitHook = () => {
    self.client = undefined;
    console.log("Intentional Reconnecting to Registry");
  ...
  if(this.connected) {
    ...
    console.log("Attempt to restart connection when there is no connection.");
  }
}


MQTTRegistry.prototype.disconnect = function() {
  this.client.end(false, undefined, (err)=>{console.log("asdkasjdlkasjdasl")});
  this.connected = false;
}
...
MQTTRegistry.prototype._prepareForEvents = function () {
    ...
    if (!connack.sessionPresent) {
      ...
      for(var machType in self.subscribedAttrs) {
        for(var attr in self.subscribedAttrs[machType]) {
          self.attrsToSubTo[machType] = self.subscribedAttrs[machType][attr];
        }
      }
  ...
  this.client.on("error", function (error) {
    console.log(`WARNING: mqtt client (${self.brokerUrl}) received an error: ${error}`);
  });
};
