...
MQTTRegistry.prototype.restartConnection = function(
    ...
    // This is a consumable exit hook, it will only be called once.
    self.customExitHook = () => {
    ...
    // We have to handle the edge case where we request to restart a connection before we have managed to
    // succesfully connect.
    if(this.connected) {
	// This is the simple case. Just disconnect, the customExitHook will initiate
	this.disconnect();
    } else if(this.client != undefined) {
	// We have attempted to start a connection but we have not succesfully connected at this point
	// we enable the cancelConnection flag, this will do an early exit as soon as the connection 
	// event is received.
	this.cancelConnection = true;
    } else {
	// This is the case where we have not yet attempted to connect (i.e. registerAndDiscover has not been called)
...
// This is used to delete retained messages (under the relevant app id topic) from a broker
// The function for now should only ever be called on the local broker for the first connection
// of a node's session.
...
MQTTRegistry.prototype._prepareForEvents = function (cleanupOnConnect) {
	...
	// Used when attempting to reconnect (generally for the reason of changing last wills.)
	...
	if(self.cancelConnection) {
	    ...
	    // This is a consumable flag and will not be called again unless explicetly
	    // re-enabled.
	...
	// This is for local broker initial connections.
	if(cleanupOnConnect)
	    await self.cleanup();
	...
	// That makes this whole if/else useless. @CLEANUP
