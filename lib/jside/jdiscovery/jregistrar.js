...
Registrar.prototype.registerAndDiscover = function (options) {
  ...
  if (!this.started) {
    ...
    // this should never run to be honset.
    if (this.localMqttClient && !this.connectedRegistries.has(this.id)) {
      this.localMqttClient.registerAndDiscover(this.localMqttBrokerUrl);
      registeredClients.add(this.id);
      this.connectedRegistries.set(this.id, this.localMqttClient);
    }
...
Registrar.prototype.connectToNewMQTTBroker = function (mqttBrokerUrl, nodeId, machType, will) { 
  if(this.connectedRegistries.has(nodeId)){
    return this.connectedRegistries.get(nodeId).client;
  }

  // unimplemented feature
  assert(will == undefined);

  mqttRegistry = new MQTTRegistry(
    this.app,
    this.machType,
    this.id,
    this.port,
  ...
  mqttRegistry.registerAndDiscover(mqttBrokerUrl);
