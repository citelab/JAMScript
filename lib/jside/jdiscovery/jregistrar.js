...
Registrar.prototype.registerAndDiscover = function (options) {
  ...
  if (!this.started) {
    ...
    if (this.globalMqttClient &&
      ...
      this.globalMqttClient.registerAndDiscover(constants.mqtt.brokerUrl, 'none');
    }
...
Registrar.prototype.cleanupLocalBroker = async function () {
  this.localMqttClient.bypassMessageProcessing = true;
  ...
  function cullMessages(topic, message) {
    if(message!="") {
      ...
      that.localMqttClient.client.publish(topic, null, {qos: 0, retain: true}, (err) => {
        if(err) {
  ...
  this.localMqttClient.client.on("message", cullMessages);

  await new Promise((resolve) => {
    this.localMqttClient.client.subscribe(`${this.app}/#`, (err, granted) => {
      if(err) {
  ...
  this.localMqttClient.client.off("message", cullMessages);

  await new Promise((resolve) => {
    this.localMqttClient.client.unsubscribe(`${this.app}/#`, (err) => {
      if(err) {
  ...
  this.localMqttClient.bypassMessageProcessing = false;
}
...
Registrar.prototype.connectToLocalMQTTBroker = function (mqttBrokerUrl, nodeId, machType, will=undefined) {
  ...
  // TODO: standardize names here REFACTOR!
  ...
  mqttRegistry.registerAndDiscover(mqttBrokerUrl, will);
  ...
  //console.log(this.availableRegistries);
  //console.log(this.connectedRegistries);

  return mqttRegistry;
}

Registrar.prototype.connectToNewMQTTBroker = function (mqttBrokerUrl, nodeId, machType, will=undefined) {
  if(this.connectedRegistries.has(nodeId)){
    let entry = this.connectedRegistries.get(nodeId);
    entry.connections++;
    return entry.mqttRegistry;
...
Registrar.prototype._installDefaultCallbacksToMqttClient = function (
  ...
  if (mqttClient) {
    ...
    mqttClient.on("client-offline", function () {
      ...
      //console.log(self.availableRegistries);
      //console.log(self.connectedRegistries);

      // Fix dangling connections
      if(self.connectedRegistries.has(mqttClientId)) {

        // TODO: fix this, If we get the down notificaiton first, then its likely that the available registries
        // will already which makes this an error!
        ...
        //console.log("Hmmm..." + self.connectedRegistries);
        //self.connectedRegistries.delete(mqttClientId);
        //console.log(self.connnectedRegistries);
