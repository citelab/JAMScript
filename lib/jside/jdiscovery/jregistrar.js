...
function Registrar(app, machType, id, port, loc, config) {
    ...
    if (!config || !config.protocols || config.protocols.mqtt) {
        ...
        this.mqttRegistry = new MQTTRegistry(app, machType, id, port, 1, 1); //subQos, pubQos);
        noProtocols = false;
    }
...
Registrar.prototype.registerAndDiscover = function(options) {
    if (options) {
        ...
        if (options.attrsToAdd) {
            console.log("Attributes to add...", options.attrsToAdd);
...
Registrar.prototype._respondToAttrRemovalEvent = function(self, attr, event, nodeId, protocol) {
    ...
    if (self.discoveries && self.discoveries[attr] && self.discoveries[attr][nodeId])
