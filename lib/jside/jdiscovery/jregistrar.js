...
Registrar.prototype.registerAndDiscover = function (options) {
  if (options) {
    ...
    if (options.attrsToDiscover) {local
      this.discoverAttributes(options.attrsToDiscover);
    }
  }

  // It is JAMCore's responsibility to initialize our local registry connection
  if(this.localMqttClient == undefined) {
    assert(this.connectedRegistries.has(this.id));
    this.localMqttClient = this.connectedRegistries.get(this.id);
  }
...
Registrar.prototype.connectToNewMQTTBroker = function (mqttBrokerUrl, nodeId, machType, will=undefined) { 
  if(this.connectedRegistries.has(nodeId)){
    return this.connectedRegistries.get(nodeId).client;
  }
  ...
  this.connectedRegistries.set(nodeId, mqttRegistry);
  
  return mqttRegistry.client;
}
/**
...
Registrar.prototype.connectToNewLocalRegistry = function (
  ...
  var doInitialRegistrySetup = false;
  if (this.connectedRegistries.has(localRegistryId)) {
    mqttClient = this.connectedRegistries.get(localRegistryId);
  } else {
    doInitialRegistrySetup = true;
  ...
  if (mqttClient) {
    ...
    mqttClient.on("client-offline", function () {
      ...
      if(self.availableRegistries.has(mqttClientId)) {
        machTypes = Object.keys(self.availableRegistries.get(mqttClientId).sourceMap);
