...
function Registrar(
  ...
  //TODO: this should probably NodeCache Responsibility
  ...
  //TODO: refactor from maheshes feedback
  this.availableRegistries = new Map();
  this.connectedRegistries = new Map();
  this.connectedRegistries.set(id, this.localMqttClient);
  ...
  if (this.machType == constants.globals.NodeType.FOG ||
    this.machType == constants.globals.NodeType.DEVICE) {
...
Registrar.prototype.registerAndDiscover = function (options) {
  if (options) {
    ...
    if (options.attrsToAdd) {
      this.setStatuses(options.attrsToAdd);
    }
  ...
  if (!this.started) {
    ...
    //TODO: should be the responsibility of the NodeCache
...
Registrar.prototype.connectToNewLocalRegistry = function (
  ...
  this.connectedRegistries.set(localRegistryId, mqttClient);
...
Registrar.prototype.connectToNewFog = function (nodeId) {
  const fog = this.availableRegistries.get(nodeId);
  assert(fog.machType == constants.globals.NodeType.FOG); //TODO: handle better
  const ip = fog.ip;
  const port = fog.port;
  ...
  //TODO: move to nodecache
  this.connectedRegistries.set(nodeId, mqttClient);
};

Registrar.prototype.disconnectFromNode = function (nodeId) {
  if (this.connectedRegistries.has(nodeId)) {
    this.connectedRegistries.get(nodeId).mqttClient.disconnect();
...
Registrar.prototype._installDefaultCallbacksToMqttClient = function (
  ...
  if (mqttClient) {
    ...
    mqttClient.on("client-offline", function () {
      ...
      //console.log(JSON.stringify(self.availableRegistries.get(mqttClientId)));
      let machTypes = Object.keys(self.availableRegistries.get(mqttClientId).sourceMap);

      self.availableRegistries.delete(mqttClientId);
      self.connectedRegistries.delete(mqttClientId);

      self.emit("client-offline", mqttClientId, mqttClientUrl, machTypes, { //TODO: change these parameters
...
Registrar.prototype._respondToDiscoveryUpEvent = function (
  ...
  if (this.availableRegistries.has(nodeId)) {
    ...
    //this.availableRegistries.set(nodeId, registryRecord);
    console.log(registryRecord);
  } else {
...
Registrar.prototype._respondToDiscoveryEvent = function (
  ...
  // New Generalised Handling TODO: reorder for likely hits
  ...
  if(eventType == "up") {
    success = this._respondToDiscoveryUpEvent(
      self, attr, event, machType, nodeId,
      value, dedupeId, mqttClientUrl, mqttClientId,
      nodeType);  
  } else if(eventType == "down") {
    success = this._respondToDiscoveryDownEvent(
      self, attr, event, machType, nodeId,
      value, dedupeId, mqttClientUrl, mqttClientId,
      nodeType);
  } else {
    console.log("Unknown Discovery Event: "+ event);
    return;
  }
  ...
  if (this.hostingLocalRegistry) {
    if (mqttClientUrl === constants.mqtt.brokerUrl) {
      // Unfortunately a bit of a hacky workaround at the moment:
...
Registrar.prototype.hostLocalRegistry = function () {
  ...
  this.connectedRegistries.set(this.globalRegistryId, this.globalMqttClient);
...
Registrar.prototype.TEMP_local_reg_destructor = function (options) {
  ...
  if (!this.hostingLocalRegistry) {
    console.err("ERROR: Cannot shutdown local registry if no local registry is running.")
    return;
  }
