...
Registrar.prototype.connectToNewFog = function (nodeId) {
  ...
  this.connectedRegistries.add(nodeId);
};

Registrar.prototype.disconnectFromNode = function (nodeId) {
  if (this.connectedRegistries.has(nodeId)) {
    this.availableRegistries.get(nodeId).mqttClient.disconnect();
...
Registrar.prototype._installDefaultCallbacksToMqttClient = function (
  ...
  if (mqttClient) {
    ...
    mqttClient.on("client-offline", function () {
      ...
      self.emit("client-offline", mqttClientId, mqttClientUrl, clientType, { //TODO: change these parameters
...
Registrar.prototype._respondToDiscoveryUpEvent = function (
  ...
  if (this.availableRegistries.has(nodeId)) {
    if (this.availableRegistries.get(nodeId)["timestamp"] > value["timestamp"]) {
      return false;  
    }
  }
...
Registrar.prototype._respondToDiscoveryDownEvent = function (
  ...
  self.availableRegistries.delete(nodeId);
  return true;
}
...
Registrar.prototype._respondToDiscoveryEvent = function (
  ...
  // New Generalised Handling TODO: reorder
  ...
  //   } else if (event === "fog-up") {
  //     value.machType = constants.globals.NodeType.FOG;
  //     if (this.availableRegistries.has(nodeId)) {
  //       const currElement = this.availableRegistries.get(nodeId);
  //       if (currElement["timestamp"] <= value["timestamp"]) {
  //         this.availableRegistries.set(nodeId, value);
  //       }
  //     } else {
  //       this.availableRegistries.set(nodeId, value);
  //     }
