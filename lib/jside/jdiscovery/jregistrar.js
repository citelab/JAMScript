...
Registrar.prototype.registerAndDiscover = function (options) {
  ...
  if(this.localMqttClient == undefined) {
    ...
    this.localMqttClient = this.connectedRegistries.get(this.id).mqttRegistry;
  }
...
Registrar.prototype.connectToNewMQTTBroker = function (mqttBrokerUrl, nodeId, machType, will=undefined) {
  if(this.connectedRegistries.has(nodeId)){
    ...
    entry.connections++;
    return entry.mqttRegistry.client;
  }
...
Registrar.prototype.connectToNewLocalRegistry = function (
  ...
  if (this.connectedRegistries.has(localRegistryId)) {
    let entry = this.connectedRegistries.get(localRegistryId);
    entry.connections++;
    mqttClient = entry.mqttRegistry;
  } else {
  ...
  if(doInitialRegistrySetup) {
    ...
    this.connectedRegistries.set(localRegistryId, {
      connections: 1,
      mqttRegistry: mqttClient
    });
  }
