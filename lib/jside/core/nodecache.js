...
class NodeCache {
  ...
  init() {
    ...
    let that = this;
    this.reggie.on("local-registry-up", (_1,_2,x) => {that.localRegistryMapChange(x)});
    this.reggie.on("local-registry-down", (_1,x) => {that.localRegistryMapChange(x)});
  ...
  startupLocalRegistryMapSettle(reggieState) {
    ...
    this.settleTimeoutID = setTimeout(() => {
      ...
      this.localRegistryMapChange(reggieState);
    }, settleTimeout);
  }

  // Evaluate this whenever there is a change in the local registry map
  localRegistryMapChange(reggieState) {
    // Wait for local registry map to converge
    if(!this.settleComplete)
    {
      this.startupLocalRegistryMapSettle(reggieState);
      return;
    }

    console.log(JSON.stringify(reggieState));
    if(this.reggie.connectedLocalRegistries.size == 0) {
      if(reggieState.localRegistriesMap.size == 0 &&
         !this.reggie.hostingLocalRegistry &&
         jsys.machType == constants.globals.NodeType.FOG) {
        this.reggie.hostLocalRegistry();
      }
      else
      {
        // Just choosing the first node for now.
        reggieState.localRegistriesMap.forEach((localRegistry, nodeId) => {
          if(nodeId != jsys.id){
