...
class NodeCache {
  ...
  localRegistryMapChange() {
    // Wait for local registry map to converge after initial global registry connection
      ...
      if(this.availableLocalRegistries.size < targetConnectedLocalRegistries) {
        if(!this.doingMulticastSearch) {
          ...
          this._discoverLocalRegistry().then((localRegistryHostInfo) => {
            ...
            that.availableLocalRegistries.set(localRegistryHostInfo.id, that._createMapEntry(localRegistryHostInfo));
            that.doingMulticastSearch = false;
  ...
  // TODO: add more specific requirements for which local registries to connect to.
  ...
  getTheClosestFog(selfLocation) {
    let result = {
        fogId: undefined,
        fogInfo: undefined
    };
    let closestDistance = Number.MAX_SAFE_INTEGER;
    this.availableFogs.forEach((value, key) => {
  ...
  getTheClosestLocalRegistry (localRegistriesMap, selfLocation) {
    let result = {
    ...
    let closestDistance = Number.MAX_SAFE_INTEGER;
    this.availableLocalRegistries.forEach((value, key) => {
