...
const Registrar = require("../jdiscovery/jregistrar");
const { Random, MersenneTwister19937 } = require("random-js");
const udp = require("dgram");
const cbor = require("cbor-x");
const random = new Random(MersenneTwister19937.autoSeed());

let activefogs, otherfogs, clouds, jsys;
...
class NodeCache {
  ...
  cloudDown(id) {
    ...
    } else clouds.delete(id);

    if (this.currentCloud === undefined) {
      if (clouds.size > 0) this.currentCloud = __getACloud(clouds);
    }
  }
}
...
function distanceToFog(info) {
  if (info.loc !== undefined)
    return helper.geo2Distance(
      jsys.long,
      jsys.lat,
      info.loc.long,
      info.loc.lat
    );
  else return helper.geo2Distance(jsys.long, jsys.lat, info.long, info.lat);
}
