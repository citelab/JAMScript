...
class NodeCache {
  ...
  init() {
    ...
    if (this.machineType === constants.globals.NodeType.DEVICE) {
      this._bootstrappingNodeToFindLocalRegistries();
    } else if (this.machineType === constants.globals.NodeType.FOG) {
      this._bootstrappingNodeToFindLocalRegistries();
    } else if (this.machineType === constants.globals.NodeType.LOCAL_REGISTRY) {
      const localRegistryMqttBrokerIp = jsys.mqtt.server;
      const localRegistryMqttBrokerPort = jsys.mqtt.port;
