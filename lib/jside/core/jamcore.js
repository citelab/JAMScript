...
class JAMCore {
    ...
    async startAuxProcessors(server) {
        ...
        sock.on('message', function(topic, buf) {
            ...
            if (Array.isArray(qmsg)) {
                for (let i = 0; i < qmsg.length; i++) {
                    let msg = qmsg[i]
                    msg.taskid = Number(msg.taskid)
                    auxmessageProcessor(that, sock, topic, msg);
                }
            } else {
        ...
        sock.on('message', function(topic, buf) {
            ...
            if (Array.isArray(qmsg)) {
                for (let i = 0; i < qmsg.length; i++) {
                    let msg = qmsg[i]
                    msg.taskid = Number(msg.taskid)
                    messageProcessor(that, sock, topic, msg);
                }
            } else {
                qmsg.taskid = Number(qmsg.taskid);          // to convert from bigint
            ...
            if ((msg.cmd > CmdNames.EXEC_CMDS_BEG) && (msg.cmd < CmdNames.EXEC_CMDS_END)) {
                ...
                switch (msg.cmd) {
                    case constants.CmdNames.REGISTER:
                        await jcore.jadmin.registerProcessor(sock, msg);
                        break;
                    case constants.CmdNames.GET_CLOUD_FOG_INFO:
                        await jcore.jadmin.infoProcessor(sock);
                        break;
