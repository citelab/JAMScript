    ...
    async run() {
        ...
        if (this.jamsys.machtype === globals.NodeType.DEVICE)
            this.me = this.makeServer(this.jamsys.id, this.jamsys.mqtt.server, cmdOpts.port, this.jamsys.machtype, 
                                        {topic: "/" + cmdOpts.app + "/admin/announce/all", payload: cbor.encode({cmd: CmdNames.EXIT}), qos: 1, retain: 0});
        else 
            this.me = this.makeServer(this.jamsys.id, this.jamsys.mqtt.server, cmdOpts.port, this.jamsys.machtype);
            ...
            if ((msg.cmd > CmdNames.EXEC_CMDS_BEG) && (msg.cmd < CmdNames.EXEC_CMDS_END)) {
                ...
                if (rmsg.cmd === CmdNames.MEXEC_ACK || rmsg.cmd === CmdNames.REXEC_ACK) {
                    ...
                    if (rmsg.cmd === CmdNames.REXEC_ACK)
                        fmsg = JAMP.createRemotePutRes(rmsg.taskid, this.jamsys.id, rmsg.nodeid, res);
                    else 
                        fmsg = JAMP.createMachPutRes(rmsg.taskid, this.jamsys.id, rmsg.nodeid, res);
