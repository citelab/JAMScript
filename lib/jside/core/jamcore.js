    ...
    switch (topic) {
        case '/' + cmdOpts.app + '/requests/up':
            ...
            if ((msg.cmd > CmdNames.EXEC_CMDS_BEG) && (msg.cmd < CmdNames.EXEC_CMDS_END)) {
                ...
                // find the handle for the admin process for this worker
                let ahandle = jcore.jadmin.getAdminProcessor(msg);
                if (ahandle !== undefined) {
                    // use the handle to process the incoming message
                    let fmsg = await ahandle.next(msg);
                    if (fmsg.cmd !== CmdNames.DONE)
                        sock.publish('/' + cmdOpts.app + '/announce/down', cbor.encode(fmsg));
                }
            }
        break;
