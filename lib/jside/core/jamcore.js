...
class JAMCore {
    constructor(jsys, reggie) {
        ...
        this.ahandle = undefined;
        this.adminProcNotRunning = true;
    ...
    processWorkerMsg(r) {
        switch (r.cmd) {
            case CmdNames.REXEC:
                this.jclient.remoteTaskExec(r.fn_name, r.fn_argsig, r.params, this.jamsys.id, r.taskid, r.results);
                break;
            case CmdNames.MEXEC:
            ...
            case CmdNames.MEXEC_RES:
                this.processWorkerResults(r.taskid, r.res_sig, r.data);
                break;
    ...
    processWorkerResults(id, res_sig, data) {
        let ientry = this.intasktbl.findExact(id);
        if (ientry !== undefined && ientry.callback !== null) {
            ientry.state = INQ_States.COMPLETED;
            ientry.res_sig = res_sig;
            ientry.results = data;
    ...
    switch (topic) {
        case '/' + cmdOpts.app + '/requests/up':
            ...
            if ((msg.cmd > CmdNames.EXEC_CMDS_BEG) && (msg.cmd < CmdNames.EXEC_CMDS_END)) {
                if ((msg.cmd >= CmdNames.MEXEC) && (msg.cmd < CmdNames.MEXEC_DONE)) {
                    for await (const fmsg of jcore.jdaemon.requestProcessor(msg)) {
                        if (fmsg.cmd !== CmdNames.DONE)
                            sock.publish('/' + cmdOpts.app + '/replies/down/j', cbor.encode(fmsg));
                    }
                } else if (msg.cmd >= CmdNames.REXEC && msg.cmd < CmdNames.REXEC_DONE) {
                    for await (const fmsg of jcore.jdaemon.requestProcessor(msg)) {
                        if (fmsg.cmd !== CmdNames.DONE)
                ...
                switch (msg.cmd) {
                    case constants.CmdNames.REGISTER:
                        // there is a single admin process handle.. if it is not yet started, start it
                        if (jcore.ahandle !== undefined)
                            jcore.ahandle = await jcore.jadmin.getAdminProcessor(msg);
                        break;
                    case constants.CmdNames.GET_CLOUD_FOG_INFO:
                        if (jcore.ahandle !== undefined && jcore.adminProcNotRunning) {
                            // use the handle to process the incoming message
                            jcore.adminProcNotRunning = false;
                            do {
                                rmsg = await jcore.ahandle.next();
                                if (!rmsg.done)
                                    sock.publish('/' + cmdOpts.app + '/announce/down', cbor.encode(rmsg.value));
                            } while (!rmsg.done)
                        }
                        break;
                    case constants.CmdNames.PONG:
                        // process the PONG.. timeout the entry
                        await jcore.jadmin.processPong(msg);
                    break;
                }
            }
        break;
