...
class JAMCore {
    ...
    async run() {
        ...
        if (this.jamsys.machtype === globals.NodeType.DEVICE)
            this.me = this.makeServer(this.jamsys.id, this.jamsys.mqtt.server, cmdOpts.port,
                                      {topic: "/" + cmdOpts.app + "/announce/down", payload: cbor.encode({cmd: CmdNames.STOP}), qos: 1, retain: 0});
        else
            this.me = this.makeServer(this.jamsys.id, this.jamsys.mqtt.server, cmdOpts.port);

        this.initializeMainProcessors(this.me.serv);
        await this.jamsys.reggie.cleanupLocalBroker();
    ...
    makeServer(id, ip, port, machType, will) {
        ...
        if(id == this.jamsys.id) {
            ...
            server.serv = this.reggie.connectToNewMQTTBroker(`${ip}:${port}`, id, machType, will).client;
        }
        return server;
    }
