...
class JAMCore {
    ...
    constructor(jsys) {
        ...
        this.ncache = this.getNcache(this.jamsys.link);
        this.fogs = new Array();
    ...
    addWorker(worker) {
        ...
        this.worker.postMessage([{cmd: CmdNames.SET_JSYS, data: {type: this.jamsys.machtype, tags: this.jamsys.tags}}]);
    ...
    processWorkerMsg(r) {
        switch (r.cmd) {
            case CmdNames.REXEC:
                this.jclient.remoteTaskExec(r.fn_name, r.params, this.id, r.taskid, r.results);
                break;
            case CmdNames.MEXEC:
            ...
            case CmdNames.MEXEC_ACK:
                this.processWorkerAck(r.nodeid + r.taskid);
                break;
            case CmdNames.REXEC_RES:
            case CmdNames.MEXEC_RES:
                this.processWorkerResults(r.nodeid + r.taskid, r.data);
                break;
            case CmdNames.REXEC_ERR:
                this.processWorkerError(r.nodeid + r.taskid, r.data);
                break;
            case CmdNames.SET_CONF:
                if (r.subcmd === globals.SubCmdName.SET_LAT)
                    this.jamsys.setLat(r.data);
                else if (r.subcmd === globals.SubCmdName.SET_LONG)
                    this.jamsys.setLong(r.data);
            break;
            default:
            ...
            if (qmsg.cmd !== undefined) {
                switch (qmsg.cmd) {
                    case CmdNames.WHERE_IS_CTRL:
                        let rmsg = {cmd: CmdNames.HERE_IS_CTRL, data: {host: that.jamsys.mqtt.server, port: that.jamsys.mqtt.port}};
    ...
    getNcache() {
        if (this.ncache === undefined) {
            // FIXME: Put these numbers in the constant table..
            this.ncache = new NodeCache(200, 2, 2000, this.jamsys.link);
        }
        return this.ncache;
    }
    ...
    registerFuncs(machbox) {
        machbox.forEach((val, key) => {
            this.registerCallback(key, null, val.arg_sig, val.sideeff, val.results, val.reuse, val.cond);
        });
    }
    ...
    processWorkerAck(id) {
        ...
        if (ientry !== undefined) {
            ientry.callback(INQ_States.STARTED)
        }
    }

    processWorkerError(id, data) {
        let ientry = this.itaskq.get(id);
        if (ientry !== undefined) {
            ientry.callback(INQ_States.ERROR, data)
        }
    }
        ...
        case '/' + cmdOpts.app + '/requests/up':
            ...
            if ((msg.cmd > CmdNames.EXEC_CMDS_BEG) && (msg.cmd < CmdNames.EXEC_CMDS_END)) {
                ...
                if (rmsg.cmd === CmdNames.MEXEC_ACK || rmsg.cmd === CmdNames.REXEC_ACK) {
                    let res = await jcore.jdaemon.checkExecResults(rmsg.nodeid + rmsg.taskid);
                    let fmsg = rmsg;
                    fmsg.data = res;
